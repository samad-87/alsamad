...
Per `ADR-0009` (superseding `ADR-0007`'s single-migration rule), KE-2 is delivered as two separately atomic, sequential forward-only migrations. `KE-2A` adds `topics` after `locales` and `editorial_users` physically exist; its migration failure rolls back only the `topics` table, its triggers, constraints, and indexes. `KE-2B` adds `content_topics` after `KE-2A` is complete and `quran_ayahs`, `devotional_items`, `content_items`, and `editorial_users` physically exist; its migration failure rolls back only the `content_topics` table, its triggers, constraints, and indexes. Migration `0010` remains reserved for M6 and is not reassigned to either unit. Committed migrations `0000`–`0009` and all intervening Release-1 migrations remain byte-unchanged. Each unit's migration number is assigned mechanically at its own execution from the then-authoritative forward-only sequence; production uses no destructive down migration for either unit.
