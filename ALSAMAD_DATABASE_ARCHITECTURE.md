...
| `commercial_use_decision`                                             | explicit boolean/decision reference                                                                      | Must be resolved; denied is valid only when the intended operation performs no commercial use, while unknown blocks.                        |
| `in_application_display_decision`                                     | explicit boolean/decision reference                                                                      | Must be approved when the intended operation serves or displays content inside ALSAMAD; denied or unknown blocks that operation.            |
| `standalone_redistribution_decision`                                  | explicit boolean/decision reference                                                                      | Must be resolved; denied is valid only when the intended operation performs no standalone redistribution, while unknown blocks.             |
...
Commercial-use and standalone-redistribution decisions must be explicitly resolved, but a known denied value is compatible with an intended operation that exercises neither capability. Unknown remains fail-closed, and no operation may exercise a denied capability. Historical manifests retain their original field names, schema, canonical bytes, and checksum meaning. Corrected ARC-001 manifests use a new schema version; no historical manifest checksum is reinterpreted. This requirement separates rights evaluation only and does not resolve the distinct immutable-manifest/run-evidence question.
