...
Constraints are `UNIQUE (provider_code, license_key, version)`; lowercase provider code; nonblank key, version, name, and attribution; valid closed vocabularies; valid HTTPS URL when present; coherent retention fields; and ordered effective dates. Indexes are `(status, effective_from, effective_until)` and `(provider_code, license_key)`. `id`, `provider_code`, `license_key`, `version`, and `effective_from` are immutable after insert.

A license row represents exactly one provider/legal revision; `provider_code` plus `license_key` plus `version` identifies that revision under this unique-constraint identity model. Once `status` first reaches `active` — the point at which the revision may be relied upon by an edition, publication, or import manifest — `rights_scope`, `attribution_text`, `terms_url`, `retention_policy`, `retention_days`, `in_application_display_allowed`, `standalone_redistribution_allowed`, `derivatives_allowed`, and `effective_until` become immutable alongside the already-frozen identity columns. Only `status` and `updated_at` may still change after that point, preserving the existing expiry/revocation/withdrawal lifecycle without rewriting the historical legal content a prior import or publication relied upon. A later legal or provider revision is recorded as a new license row under a new `version`, never as an edit to an existing active row.

Activation requires nonblank attribution and a currently effective window. Revocation or expiry prevents new publication but preserves both the historical row and its now-frozen legal content unchanged; content published or imported under the prior terms remains traceable to exactly what was approved at that time, independent of any later license revision. Deletion is restricted while referenced; licenses are retained for the life of every dependent edition, publication record, and import manifest. Contract documents, credentials, and secret terms are prohibited.
...
| `is_active_release`        | `boolean`       | Required; default `false`             | Whether this is the currently served default edition for `work_id`; see §5.3.12.           |
| `created_at`               | `timestamptz`   | Required; default `current_timestamp` | UTC creation time.                                                                         |
| `updated_at`               | `timestamptz`   | Required; default `current_timestamp` | UTC last pre-publication update.                                                           |

Constraints are `UNIQUE (work_id, edition_key, version)`, `UNIQUE (provider_code, provider_edition_id, import_version)`, closed publication state, lowercase codes, script format, nonblank aliases, a 64-character lowercase SHA-256 checksum, JSON object size at most 16 KiB, coherent `published_at`, and `CHECK (NOT is_active_release OR publication_state = 'published')`. Indexes are `work_id`, `license_id`, `(publication_state, work_id)`, `(provider_code, provider_edition_id)`, and a `UNIQUE` partial index on `work_id` `WHERE is_active_release AND publication_state = 'published'` (§5.3.12). Publication is permitted only through a trigger that confirms the referenced license is `active`, effective at `published_at`, permits storage, has `in_application_display_allowed = true`, and has not expired or been revoked. `standalone_redistribution_allowed` is not a publication prerequisite. Once an edition first enters `published`, every column except `publication_state`, `updated_at`, `is_active_release`, and a transition from `published` to `withdrawn` is immutable; withdrawal cannot return to publication and must clear `is_active_release` in the same transaction. A correction creates a new edition version. Time-limited source data must be withdrawn and deleted according to its license unless separate durable rights exist; edition identity and required attribution evidence remain retained, while licensed payload removal is handled through restricted recovery procedures.
...
| `is_active_release` | `boolean`       | Required; default `false`             | Whether this is the currently served default translation for `locale_id`; see §5.3.12.   |
| `created_at`        | `timestamptz`   | Required; default `current_timestamp` | Creation time.                                                                           |
| `updated_at`        | `timestamptz`   | Required; default `current_timestamp` | Last authorized pre-approval or withdrawal update.                                       |

Constraints are uniqueness of `edition_id`, closed review status, coherent review time, nonblank attribution/methodology, methodology length at most 8 KiB, and `CHECK (NOT is_active_release OR review_status = 'approved')`. A trigger requires the generic edition to belong to the approved Quran work, use the locale language, reference the same license, and carry its provider alias, import version, manifest checksum, and publication lifecycle in the M4 fields. Indexes are `(locale_id, review_status)`, `license_id`, `review_status`, and a `UNIQUE` partial index on `locale_id` `WHERE is_active_release AND review_status = 'approved'` (§5.3.12). Publication requires `review_status = 'approved'`, active license and attribution, approved source decision, complete verse reconciliation, and the generic edition to be published. Published identity and attribution freeze except `is_active_release`; withdrawal is one-way, propagates to all translation texts, and must clear `is_active_release` in the same transaction.
...
### 5.3.12 Release selector and activation (ARC-005)

§5.3.9 requires "a stable release/version selector outside the canonical table count so readers cannot observe a mixed provider version." This section fixes that selector's physical shape. It adds no table; `editions` and `quran_translation_editions` (already counted in the M4/M5 catalog) each carry it directly.

**Canonical release identity.** A release is not a new identity. It remains exactly the single approved Quran `work_id` and its edition-independent `quran_surahs`/`quran_ayahs` canonical structure (§5.3.1–§5.3.3). The selector answers a narrower question: which already-published `editions` row, and which already-approved-and-published `quran_translation_editions` row per locale, is currently served by default.

**Two independent selector domains.** Arabic edition activation and translation edition activation are independent. `quran_ayah_texts` and `quran_translation_texts` both key off the edition-independent `quran_ayahs.id`, so no translation row's eligibility depends on which Arabic edition is active, and activating, rolling back, or withdrawing the active Arabic edition never requires or implies a corresponding translation change, or the reverse. `editions.is_active_release` is scoped `WHERE is_active_release AND publication_state = 'published'`, unique per `work_id`. `quran_translation_editions.is_active_release` is scoped `WHERE is_active_release AND review_status = 'approved'`, unique per `locale_id`. Both are declared and enforced independently.

**Zero-active fails closed.** A query for the active Arabic release, or the active translation for a given locale, that matches zero rows must report an honest unavailable state, matching the honest empty/pending/available pattern already established for content sourcing elsewhere in this repository. No implicit "most recently published" or "lowest ID" fallback is permitted in place of a genuinely unset selector.

**Selector signal versus live eligibility.** `is_active_release = true` identifies which already-eligible row governed activation most recently chose; it is **not, by itself, proof the row is currently servable.** Eligibility also depends on facts outside the flagged row — the backing `licenses.status` and `effective_until` (§5.2.2), for a translation its backing generic `editions.publication_state` (§5.3.6), and `locales.enabled` — none of which a same-row `CHECK` or partial unique index can observe, and `effective_until` elapsing requires no write at all. Every public Quran read must therefore **re-derive the full current eligibility chain at read time**, in the same authoritative bounded query or transactionally consistent read that resolves the active candidate, exactly as §5.3.4/§5.3.7 already require for text-level serving independent of this selector. If the active candidate fails that live check, serving fails closed the same as the zero-active case: no silent fallback to a different published edition or approved translation.

**Atomic activation, rollback, and withdrawal.** Activation, rollback, and withdrawal are one primitive, not three. Activating edition `E` for work `W` (or, symmetrically, a translation edition for a locale) is a single transaction: first clear any prior active row for that scope, then set the new one. A partial unique index only forbids two simultaneously active rows in one scope; a momentary zero-active state between the two statements is permitted by the index and is never visible outside the transaction under standard `READ COMMITTED` isolation. A failed switch leaves the prior selector state unchanged. Governed activation must validate the candidate's full live eligibility chain at the moment of activation, but that check is a point-in-time gate, not a standing guarantee — read-time revalidation above remains mandatory regardless. Rollback to a previously active, still-published/approved row is the same primitive invoked with the prior row's ID; no separate rollback table or snapshot is introduced. Withdrawal (a `published`/`approved` row leaving that state) must clear `is_active_release` in the same transaction; `CHECK (NOT is_active_release OR publication_state = 'published')` on `editions`, and the equivalent tied to `review_status = 'approved'` on `quran_translation_editions`, make it physically impossible to leave a withdrawn row marked active.

**Upstream eligibility loss.** License revocation, license time-expiry, generic-edition withdrawal, and locale disablement remain the existing, unblocked, one-way safety operations §5.2.2/§5.2.4/§5.3.6 already define; this section adds no cross-layer trigger on `licenses` or `editions` that either blocks those operations or automatically clears `is_active_release` when they occur — no such trigger exists elsewhere in this schema, and time-expiry has no write event for any trigger to observe. Instead, read-time eligibility re-derivation (above) immediately stops an affected candidate from being served, and the Publisher/governed-activation workflow (§30.1, §29.2 of `ALSAMAD_ADMIN_ARCHITECTURE.md`) is responsible for explicitly deactivating, withdrawing, rolling back, or selecting a replacement once the loss is noticed.

**Concurrency.** A `UNIQUE` partial index per selector scope is the sole concurrency mechanism: `CREATE UNIQUE INDEX ... ON editions (work_id) WHERE is_active_release AND publication_state = 'published'` and the equivalent on `quran_translation_editions (locale_id) WHERE is_active_release AND review_status = 'approved'`. Two concurrent activation transactions targeting the same scope cannot both commit; whichever commits second violates the index and aborts. No advisory lock or application-level mutex is required or authorized in its place.

**Immutability exemption.** `is_active_release` is exempt from each table's post-publication immutability freeze, alongside the existing `publication_state`/`review_status` and `updated_at` exemptions (§5.2.4, §5.3.6). It carries no identity, legal, or canonical-text meaning and is expected to keep changing after publication.

**Ownership.** The Quran module owns `is_active_release` on both tables as part of its existing ownership of approved Quran renderings and translations (`ALSAMAD_IMPLEMENTATION_ROADMAP.md` module ownership matrix). The existing Publisher role (`ALSAMAD_ADMIN_ARCHITECTURE.md` §30.1) exercises its existing guarded-publication authority to mutate it; no new role, table, or owning module is introduced.

**Future read-path contract.** `src/lib/quran/content/db-source.ts` is not modified by this documentation-only section, but its future implementation must: resolve the active candidate per selector domain; in the same bounded, transactionally consistent read, join/re-derive every current eligibility dependency above; return content only if the candidate remains fully eligible; otherwise fail closed; never independently select a different "latest published" fallback; and never mix content across inconsistent selector states between the two domains.

This section documents `REG-0013`/`ADR-0006`. It authorizes no migration, no schema change, and no implementation; migration `0008` is reserved for it under `ALSAMAD_IMPLEMENTATION_ROADMAP.md`.
...
| -------------------- | ---------------------- | ------------------ |
| `users`              | Expanded V1 physical contract approved; inert implementation COMPLETE/PASS | Durable shared account root fixed by `REG-0029`/`ADR-0012` and implemented only within `REG-0030`; no broader account authority. |
| `user_identities`    | Expanded V1 physical contract approved; implementation blocked | Provider-neutral authentication link fixed by `REG-0032`/`ADR-0014`; an external subject is never the durable ALSAMAD account identity. |
| `user_sessions`      | Expanded V1 physical contract approved; implementation blocked | Provider-neutral, transport-neutral five-field session record fixed by `REG-0035`/`ADR-0016`; no table, row, credential, runtime, or API authority. |
...
The Core Release 1 physical count remains exactly 30. The approved Expanded V1 account-root physical contract remains one `users` root; the historical projection from 30 to 35 is not current implementation authority or a frozen activation count. `user_identities`, `user_sessions`, `user_preferences`, and `user_saved_items` remain outside that root. The separate `user_sessions` physical contract is approved under `REG-0035`/`ADR-0016`, but it remains unimplemented and outside the Core Release 1 physical count.
...
The root's sole purpose is stable shared ALSAMAD account identity. It contains no contact, username, display, avatar, locale, provider, credential, session, recovery, preference, saved-item, bookmark, reading-position, private-routine, profile, marketing, analytics, engagement, Editorial, Talibeen, or other module field/reference. It has no outgoing or dependent foreign key and no cascade. The separate `user_identities` physical contract is approved under `REG-0032`/`ADR-0014` while implementation remains blocked; the separate `user_sessions` physical contract is approved under `REG-0035`/`ADR-0016` while database implementation, runtime, real rows/data, and production remain `BLOCKED / NOT AUTHORIZED`; and preferences and saved items remain Prepared/deferred.
...
At the `REG-0031`/`ADR-0013` architecture crossing, a future `user_identities`-like model was conceptually expected but not physically approved. `REG-0032`/`ADR-0014` now decide that separate physical contract in §9.3 without changing these architecture invariants. Implementation, rows, provider integration, API/runtime access, sessions, recovery, and real-user processing remain blocked.

## 9.3 Approved provider-neutral authentication identity linkage physical contract

`REG-0032` and accepted `ADR-0014`, dependent on `REG-0031`/`ADR-0013`, approve the minimum ALSAMAD-owned physical mapping required to preserve one stable durable-account relationship across replaceable authenticators and to enforce fail-closed ownership continuity while linkage evidence is retained. Provider/runtime-only state is insufficient for that invariant. This section freezes physical shape only; it creates no table, ORM declaration, migration, row, provider, API, runtime, session, recovery, or implementation authority.

The future table is exactly `user_identities` with exactly seven columns:

| Column | PostgreSQL type | Nullability and default | Physical contract |
| --- | --- | --- | --- |
| `id` | `uuid` | `PRIMARY KEY NOT NULL`; no database default | Application-generated UUIDv7; opaque, immutable, ALSAMAD-owned linkage-row identity; never reused. |
| `user_id` | `uuid` | `NOT NULL`; no default | Immutable reference to `users.id`. |
| `authenticator_namespace` | `varchar(128)` with `C` collation | `NOT NULL`; no default | Immutable ALSAMAD-governed lowercase ASCII namespace token; checked against `[a-z0-9][a-z0-9._-]{0,127}`. |
| `subject` | `text` with `C` collation | `NOT NULL`; no default | Immutable, non-empty, opaque authenticator/provider subject stored exactly; no universal provider maximum, contact inference, or provider-specific normalization is frozen. |
| `status` | `varchar(16)` | `NOT NULL`; no default | Explicitly supplied as exactly `active` or `retired`; operational mapping eligibility only. |
| `created_at` | `timestamptz` | `NOT NULL DEFAULT current_timestamp` | Immutable UTC mapping-creation evidence. |
| `updated_at` | `timestamptz` | `NOT NULL DEFAULT current_timestamp` | UTC status-transition evidence; changes only with a real status transition and must strictly increase. |

`ck_user_identities__id_uuidv7` enforces UUIDv7 and the RFC variant. `ck_user_identities__authenticator_namespace` closes the internal namespace syntax, and `ck_user_identities__subject_nonempty` rejects an empty subject. The namespace's 128-character limit bounds only ALSAMAD's controlled canonical namespace vocabulary and claims no provider identifier/subject limit. Namespace and subject comparisons are case-sensitive under PostgreSQL `C` collation. Unbounded `text` avoids inventing a universal provider-subject maximum. Subject semantics, provider-specific limits, validation, and normalization remain later adapter/provider governance; this contract does not trim, fold case, normalize Unicode, hash, tokenize, or encrypt the subject by assumption. Status has no default so omission cannot silently create a resolution-eligible mapping.

`fk_user_identities__user` is a non-null, `NOT DEFERRABLE` FK to `users(id)` with `ON UPDATE RESTRICT` and `ON DELETE RESTRICT`. Link rows cannot be orphaned, moved between accounts, or silently removed by account deletion. Restricted, `deletion_pending`, or `deleted` root state does not alter physical ownership; future resolution must fail closed under separately governed account-state rules. A referenced root cannot be physically removed until linkage disposition, retention, deletion-completion, security-lineage, and legal-hold obligations are separately satisfied.

`uq_user_identities__authenticator_subject` unconditionally covers (`authenticator_namespace`, `subject`) across every retained active and retired row. PostgreSQL uniqueness closes concurrent duplicate insertion and ensures one retained external authentication identity has at most one durable account mapping and cannot be reassigned through a replacement insert while the row exists. `user_id` is deliberately not unique, permitting multiple identities per account. `ix_user_identities__user_id` is justified only for FK/account-disposition integrity. The primary-key index, canonical unique index, and `user_id` index are the only indexes; no runtime lookup/performance index is approved.

The constraint is not a permanent denylist. Later separately governed lawful erasure or retention expiry may delete a row, after which this table no longer contains the canonical key or enforces post-erasure non-reassignment. Deletion grants no reassignment authority. Provider-subject recycling, verified reassignment, account-deletion edge cases, and any exceptional ownership transition remain blocked pending provider-specific research, proof, Security/privacy governance, and immutable audit evidence; without such authority, conflict or insufficient ownership evidence fails closed. No indefinitely retained hash, tombstone, or alternate identifier is authorized.

The lifecycle uses a retention-bounded inactive linkage record. `retired` is ineligible for future resolution but remains associated with its immutable account while lawfully retained. Relink can only reactivate the same retained row for the same account/namespace/subject under later proof/runtime/audit governance. Replacement creates a different canonical row and retires the prior row; no predecessor field is added. A future `trg_user_identities__integrity` rejects mutation of `id`, `user_id`, `authenticator_namespace`, `subject`, or `created_at`; rejects timestamp-only/no-op updates; and requires an explicitly supplied strictly later `updated_at` for a real status transition. Hard deletion is not ordinary unlink; retention duration, lawful erasure, deletion completion, legal holds, backups, and jurisdiction rules remain later authority.

This table is operational mapping state plus minimum retention-bounded ownership evidence, not an audit-event store. `updated_at` records only the latest current-state transition and does not preserve prior transition count/times, actor, reason, proof, request context, or immutable history. Before any real unlink, replacement, or relink, separate governance must authorize necessary immutable event evidence and its access, retention, deletion, and legal-hold treatment. No runtime mutation authority exists.

The exact contract contains no email, phone, username, display/profile/contact metadata, token, credential, password, passkey, public key, secret, session identifier, recovery data, provider payload, analytics, marketing, Editorial, Talibeen, or module data. It is personal-data-capable architecture but authorizes no personal-data processing.

A later separately authorized inert implementation could use one isolated additive migration, zero rows, zero seed/backfill, zero consumers, no API/provider/session/recovery integration, and no real identities. That possibility is not implementation authority. A later Roadmap crossing must name the exact implementation boundary and require proof of exact schema, no subject length bound, explicit status/no default, constraints, concurrency-safe retained-row uniqueness, restrictive FK, three-index limit, forbidden-field absence, migration/journal isolation, zero rows, zero consumers, API/runtime inertness, quarantine preservation, and rollback while empty and unused.

## 9.4 Approved provider-neutral authentication session physical contract

`REG-0035` and accepted `ADR-0016`, dependent on `REG-0034`/`ADR-0015` and the durable account-root authority, approve only the minimum PostgreSQL physical contract for future server-managed authentication sessions. PostgreSQL is the authoritative persistent source of truth. This section freezes physical shape only; it creates no table, ORM declaration, migration, row, credential, API, runtime, provider, Recovery, real-data, production, or successor authority. A cache or auxiliary store requires separate governance and cannot silently replace PostgreSQL as canonical authority.

The future physical entity is exactly `user_sessions`, with exactly five columns:

| Column | PostgreSQL type | Nullability and default | Physical contract |
| --- | --- | --- | --- |
| `id` | `uuid` | `PRIMARY KEY NOT NULL`; no database default | Application-generated UUIDv7 with the repository-standard RFC-variant check; ALSAMAD-owned, opaque, immutable, never reused, record identity only, and never a bearer credential or secret. |
| `user_id` | `uuid` | `NOT NULL`; no default | Immutable ownership reference to exactly one durable `users.id`. |
| `created_at` | `timestamptz` | `NOT NULL`; no default | Immutable UTC row-creation/issuance evidence and the lower temporal bound for expiry and revocation; not immutable audit history. |
| `expires_at` | `timestamptz` | `NOT NULL`; no default | Immutable UTC bounded-authority evidence; must be strictly later than `created_at`. |
| `revoked_at` | `timestamptz` | nullable; initially `NULL`; no default | `NULL` means no row-level revocation is recorded; non-null means revoked and must be at least `created_at`; one-way and immutable once set. |

`ck_user_sessions__id_uuidv7` uses the established UUIDv7 and RFC-variant integrity approach. `fk_user_sessions__user` references `users(id)`, is non-null and `NOT DEFERRABLE`, and uses `ON UPDATE RESTRICT` and `ON DELETE RESTRICT`. `user_id` cannot change. Restrictive deletion prevents account deletion from silently cascading session authority; account lifecycle and session disposition remain separately governed.

`ck_user_sessions__expires_after_creation` requires `expires_at > created_at`. `ck_user_sessions__revoked_not_before_creation` requires `revoked_at IS NULL OR revoked_at >= created_at`. A future integrity trigger named `trg_user_sessions__integrity` must reject mutation of `id`, `user_id`, `created_at`, or `expires_at`; reject clearing or rewriting a non-null `revoked_at`; permit only `revoked_at: NULL → valid timestamp`; and reject no-op lifecycle mutation. The supporting function name remains a later local implementation detail. This physical capability authorizes no mutation caller.

Lifecycle is derived rather than duplicated in mutable status. `ISSUED` is successful row creation. `ACTIVE` requires `revoked_at IS NULL`, current time before `expires_at`, and current fail-closed account/authentication/security evaluation permitting authority. `EXPIRED` means current time is at or after `expires_at`. `REVOKED` means `revoked_at IS NOT NULL`. `INVALID` is evaluation-only. There is no persisted status/lifecycle enum, `SUSPENDED`, `updated_at`, generation, or version field.

The only index classes are the primary-key index on `id` and non-unique `ix_user_sessions__user_id`. Primary-key uniqueness on `id` is the only uniqueness rule. `user_id` is deliberately not unique, so multiple concurrent session rows per account and independent row revocation remain physically possible. The `user_id` index supports locating retained account sessions for future separately governed account-wide invalidation. No expiry, revocation, token, provider, cleanup, or speculative performance index is approved.

The record identity is separate from any future bearer credential. No raw session/bearer secret is persisted. The exact five-column contract contains no `user_identity_id`, credential/token/hash/refresh/lookup/rotation field, client/device context, IP, user agent, geolocation, fingerprint, hardware or advertising identifier, device label/trust/ownership assertion, provider metadata/payload/token, contact data, assurance/trust/reputation/personhood/household field, Talibeen or Editorial identity, revocation reason, audit-event reference, cleanup metadata, or other column. Credential resolution persistence, if ever justified, requires separate governance.

Operational state is not immutable audit evidence. Audit governance is not a prerequisite to this physical decision or a separately authorized empty runtime-inert implementation, but appropriate logging/audit authority is required before real issuance; immutable event governance remains required for real revocation where existing authority requires it; and support/admin mutation requires separately governed immutable purpose-bound evidence. Numeric retention is unresolved. Real rows and processing remain blocked until applicable privacy, lawful-basis, notice, retention, erasure, deletion-completion, backup, access/export, correction, transfer/subprocessor, support-access, legal-hold, jurisdiction, Security, API/runtime, and provider gates pass.

Account-wide or authentication-state invalidation may later revoke all applicable rows while current account/security evaluation fails closed. Transaction isolation, locking, concurrent issuance serialization, generation/version, and runtime algorithms remain undecided; no generation/version column is approved. No numeric TTL, idle/absolute/sliding policy, remember-me duration, refresh lifetime, cleanup schedule, or clock-skew policy is frozen. Recovery, provider/login/signup behavior, tokens/cookies/headers/endpoints/serialization/CSRF/middleware, and all runtime behavior remain separate.

A later Owner-reviewed Roadmap crossing may consider only a zero-row, runtime-inert, verifier-backed, rollback-safe implementation after a separate readiness audit. `REG-0035`/`ADR-0016` assign no migration or implementation boundary and authorize no schema work.
...
Per supplemental `ADR-0010`, the complete lifecycle graph is `draft → approved`, `draft → retired`, and `approved → retired` only. `approved → draft`, every transition out of `retired`, a `retired → retired` no-op, and all reactivation are rejected; retirement is terminal. Retiring an approved topic preserves `approved_by` and `approved_at` unchanged. Retiring a draft leaves both null. Approval evidence is created only by `draft → approved` and is never fabricated or rewritten by retirement.

`created_at` and initial `updated_at` use their database `current_timestamp` defaults; `created_at` is immutable. Each actual localized-name replacement, approval, or retirement uses one database-owned lifecycle-event timestamp and must strictly advance `updated_at`. Approval assigns the same event timestamp to `approved_at` and `updated_at`; retirement advances only `updated_at`. No-op mutations are rejected before timestamp assignment. The lifecycle trigger derives the event time exactly as `GREATEST(clock_timestamp(), OLD.updated_at + interval '1 microsecond')`, assigns it to `NEW.updated_at`, and on `draft → approved` also assigns it to `NEW.approved_at`. Application-supplied changes to `created_at`, `updated_at`, or `approved_at` outside these database-owned assignments are rejected. PostgreSQL's one-microsecond timestamp quantum guarantees `NEW.updated_at > OLD.updated_at` even when the wall clock has not advanced.

Every topic create, localized-name replacement, approval, retirement, and canonical-key replacement verifies the required `editorial_users` actor is `active` inside the same repository-owned `READ COMMITTED` transaction as the mutation, applying §5.5.1's existing consumer rule. Lifecycle mutations lock the target topic row before state validation. Ordinary read-by-ID requires no actor and no advisory or row lock.
...
The unique constraint supplies the canonical-key index. An index on `status` supports approved-vocabulary reads. No search/vector/full-text index is authorized. `id`, `canonical_key`, `created_by`, and `created_at` are immutable after insert. Renaming replaces the complete `localized_names` map for a draft or approved topic only and requires an actual change. Canonical-key correction is one atomic repository operation: lock and retire the old draft/approved topic, then create a replacement with a new application-generated UUIDv7, its own `created_by`, `status = 'draft'`, fresh default-generated `created_at`/`updated_at`, and null approval evidence. The old topic receives its retirement `updated_at`; a retired topic cannot be corrected or reactivated. Delete is restricted while `content_topics` references the topic. Approved or retired rows are not hard-deleted by ordinary application writes, and the repository exposes no ordinary hard-delete operation.
...
Per `ADR-0009` (superseding `ADR-0007`'s single-migration rule), KE-2 is delivered as two separately atomic, sequential forward-only migrations. `KE-2A` added `topics` through committed migration `drizzle/0012_ke2a_topics.sql` after `locales` and `editorial_users` physically existed; its separately atomic rollback boundary contains only the `topics` table, its triggers, constraints, and indexes. `KE-2B` adds `content_topics` after `KE-2A` is complete and `quran_ayahs`, `devotional_items`, `content_items`, and `editorial_users` physically exist; its migration failure rolls back only the `content_topics` table, its triggers, constraints, and indexes. Migration `0010` remains reserved for M6 and is not reassigned to either unit. Committed migrations `0000`–`0009`, `0011`, and `0012` remain byte-unchanged after their respective execution. KE-2A's number was assigned mechanically at its execution; KE-2B's number remains unassigned until its own authorized execution from the then-authoritative forward-only sequence. Production uses no destructive down migration for either unit.
...
- Public Identity architecture is OPEN / APPROVED under `REG-0028`/`ADR-0011`; the minimal `users` durable account-root physical contract is APPROVED and its exact inert persistence is COMPLETE/PASS under `REG-0029`/`ADR-0012` and `REG-0030`. Authentication linkage architecture is OPEN / APPROVED under `REG-0031`/`ADR-0013`, and the provider-neutral `user_identities` physical contract is APPROVED with its inert persistence COMPLETE/PASS under `REG-0032`/`ADR-0014` and `REG-0033`. Session architecture is DECIDED under `REG-0034`/`ADR-0015`, and the provider-neutral, transport-neutral five-field `user_sessions` physical contract is APPROVED under `REG-0035`/`ADR-0016`; session persistence and runtime remain BLOCKED / NOT AUTHORIZED. Recovery persistence remains unresolved; provider selection and real-data processing remain blocked; and preferences and saved items remain Prepared/deferred.
