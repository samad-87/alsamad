...
- Public Identity architecture is OPEN / APPROVED under `REG-0028`/`ADR-0011`; the minimal `users` durable account-root physical contract is APPROVED and its exact inert persistence is COMPLETE/PASS under `REG-0029`/`ADR-0012` and `REG-0030`. Authentication linkage architecture is OPEN / APPROVED under `REG-0031`/`ADR-0013`, and the provider-neutral `user_identities` physical contract is APPROVED under `REG-0032`/`ADR-0014`; linkage implementation remains BLOCKED / NOT AUTHORIZED. Sessions and `user_sessions` remain conceptual/deferred; recovery persistence remains unresolved; provider selection and real-data processing remain blocked; and preferences and saved items remain Prepared/deferred.
