...
### REG-0036 — Public ALSAMAD authentication session runtime-inert persistence implementation authorization

**Category:** `Database`, `Security`, `API`, `Roadmap`.

**Summary:** Authorize, subject to separate explicit Owner implementation approval, one future empty, zero-real-row, provider-neutral, transport-inert, API-inert, runtime-inert, and production-inert persistence implementation of the `user_sessions` physical contract already approved by `REG-0035`/`ADR-0016`.

**Committed evidence:** `REG-0034`/accepted `ADR-0015` establish the session authority and threat boundary. `REG-0035`/accepted `ADR-0016` freeze PostgreSQL as authoritative persistent session-state storage and freeze the complete five-column `user_sessions` physical contract, derived lifecycle, credential separation, concurrency capability, exact index boundary, audit/privacy gates, and inert rollback premise. Implemented `REG-0030` and `REG-0033` establish the repository precedent for a narrowly authorized four-file, zero-row, runtime-inert persistence unit. No new ADR is required because this decision changes no material architecture or physical contract.
...
1. create `drizzle/0015_public_identity_authentication_session.sql`;
2. modify `drizzle/meta/_journal.json` only for one mechanical registration append;
3. modify `src/db/schema.ts` only for the exact `user_sessions` ORM declaration;
4. modify `scripts/db-verify.mjs` only for this unit's PostgreSQL and repository verification.

No fifth implementation file, snapshot, standalone test, dependency/package, runtime, API, provider, Recovery, UI, service, repository, or unrelated file is authorized. `0015` is the mechanically next migration after committed `0014_public_identity_authentication_linkage.sql`; the semantic suffix is exactly `public_identity_authentication_session`. If implementation discovers that any additional file or material physical decision is required, it must stop and return to governance.

**Exact adopted physical contract:** PostgreSQL remains authoritative. Create exactly `user_sessions` with exactly five columns and no sixth field: application-generated UUIDv7 `id uuid PRIMARY KEY NOT NULL` with no database default, repository-standard RFC-variant integrity, immutable, opaque, never reused, and record identity only; immutable non-unique `user_id uuid NOT NULL` with no default and a `NOT DEFERRABLE` FK to `users(id)` using `ON UPDATE RESTRICT` and `ON DELETE RESTRICT`; immutable `created_at timestamptz NOT NULL` with no default; immutable `expires_at timestamptz NOT NULL` with no default and `expires_at > created_at`; and nullable `revoked_at timestamptz` initially `NULL`, with no default, constrained by `revoked_at IS NULL OR revoked_at >= created_at`, and mutable only once from `NULL` to a valid timestamp.

**Integrity, lifecycle, indexes, and uniqueness:** Database constraints and `trg_user_sessions__integrity` must enforce the contract; application logic cannot substitute for database integrity. The trigger must reject mutation of `id`, `user_id`, `created_at`, or `expires_at`; reject clearing or rewriting non-null `revoked_at`; permit only `revoked_at: NULL → valid timestamp`; and reject no-op lifecycle mutation. No lifecycle/status column is persisted. `ISSUED`, `ACTIVE`, `EXPIRED`, and `REVOKED` remain derived exactly as in `ADR-0016`; `INVALID` remains evaluation-only and `SUSPENDED` remains absent. The exact indexes are `user_sessions_pkey` and non-unique `ix_user_sessions__user_id`; primary-key uniqueness is the only uniqueness rule. No expiry, revocation, token, provider, cleanup, or speculative operational index is authorized.

**Zero-row and inert boundary:** The migration may create no seed, backfill, retained fixture, imported, synthetic-retained, real, or production row. Transaction-scoped verifier rows are permitted only when rolled back; after verification, persistent `user_sessions` row count must equal zero. The unit has no runtime reader/writer, repository, service, issuance, validation, revocation, revoke-all, account-wide mutation, authentication middleware, job, route, endpoint, request/response contract, cookie, header, serialization, CSRF mechanism, token handling, provider integration, login/signup, Recovery, UI, deployment, or production effect.

**Credential and data-minimization boundary:** `user_sessions.id` is record identity, never bearer identity. No raw bearer/session secret, session token, token hash, refresh value, lookup secret, credential-resolution persistence, rotation lineage, or replay mechanism is stored or selected. The exact catalog must contain no status, `updated_at`, generation/version, `user_identity_id`, credential field, IP, user agent, location, fingerprint, device identifier or label, provider metadata or token, email, phone, contact identifier, assurance, trust, reputation, personhood, household, Talibeen identity, Editorial identity, audit-event reference, revocation reason, cleanup metadata, or other sixth field.

**Verification and acceptance:** Real PostgreSQL and repository verification must prove: exact table existence and five-column catalog; types, nullability, and absent defaults; application-supplied UUIDv7/RFC-variant acceptance and invalid version/variant, missing-ID, duplicate-ID, and ID-mutation rejection; missing/invalid owner rejection; FK target, actions, and non-deferrability; `user_id` immutability and acceptance of multiple rows for one user; omission and mutation rejection for `created_at` and `expires_at`; rejection of equal/earlier expiry and acceptance of later expiry; nullable and valid non-null `revoked_at`; rejection before creation; acceptance of `NULL → valid timestamp`; rejection of clearing, rewriting, and no-op lifecycle mutation; exactly two indexes with a non-unique `user_id` index and no extra uniqueness; prohibited-field and raw-secret absence; zero persistent rows; exact migration and one-entry journal append; prior-migration byte integrity; exact four-file boundary; and zero runtime/API/provider/Recovery consumers. Synthetic verification rows must be transaction-scoped and rolled back.

Required quality evidence is `npm run db:check`, `npm run typecheck`, `npm run lint`, targeted implementation-file formatting, repository tests, `npm run build`, and `git diff --check`, plus exact implementation/staged-file review, migration/journal review, rollback evidence, and disclosure of unrelated repository-wide formatting debt without widening scope. Compilation alone is insufficient.

**Rollback:** Migration failure must roll back atomically. Verification failure prevents acceptance and leaves zero retained rows. Before application, an unaccepted local implementation may be reverted within its exact boundary. After a migration is committed or applied, migration and journal history must not be rewritten; removal while `user_sessions` remains proven empty and unconsumed requires separately reviewed authority and a forward cleanup migration. No destructive production-data rollback is authorized.

**Audit, privacy, and downstream gates:** Immutable audit-event governance and numeric retention/privacy policy are not prerequisites to this empty runtime-inert implementation. Applicable audit/logging authority remains mandatory before real issuance; immutable purpose-bound evidence remains required for applicable real revocation/lifecycle and support/admin mutation. Before any real processing, applicable purpose, lawful basis, notice, retention, erasure, deletion completion, backup, access/export, correction, transfer/subprocessor, support-access, legal-hold, and jurisdiction obligations must be resolved. Recovery remains separate and downstream. Provider/login/signup and API/transport remain deferred and unauthorized. No downstream gate is weakened.
