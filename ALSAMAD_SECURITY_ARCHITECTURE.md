...
`REG-0032`/`ADR-0014` approve the physical contract only for a possible future zero-row `user_identities` mapping. Its anti-takeover boundary is database-enforced while linkage evidence is retained: immutable `user_id`; a non-null, non-deferrable, update/delete-restrictive FK to `users.id`; and an unconditional unique (`authenticator_namespace`, `subject`) constraint across retained active and retired rows. The unique constraint, not application pre-checking, closes concurrent duplicate insertion. A collision fails and creates no alternate/conflict mapping, merge, transfer, reassignment, or selected winner. Multiple identities per account remain allowed because `user_id` is not unique.

The linkage row has immutable ALSAMAD UUIDv7 identity, account assignment, namespace, opaque subject, and creation evidence. Its explicit checked `active`/`retired` status has no default. Retirement removes future physical resolution eligibility but preserves account ownership continuity while the inactive row is lawfully retained; relink can only reactivate that same row for the same account and canonical identity under later proof/runtime/audit governance. Restricted, deletion-pending, and terminal account states never rewrite ownership, and future resolution must fail closed against account state. The restrictive FK blocks physical root deletion until separately governed linkage disposition, lineage, retention, deletion-completion, and legal-hold requirements pass.

Retention is not permanent. Later lawful erasure or retention expiry may remove the canonical row, after which this table cannot enforce post-erasure non-reassignment. Deletion grants no reassignment authority and no permanent denylist, hash, tombstone, or alternate retained identifier is approved. Provider-subject recycling and every exceptional reassignment path remain unsupported and blocked pending provider-specific research, proof requirements, Security/privacy review, and immutable audit evidence; absent that authority, conflict or insufficient evidence fails closed.

Operational state remains separate from security/audit evidence. `updated_at` represents only the latest current-state transition; `user_identities` preserves no prior transition count/times, actor, reason, proof, request context, IP, user agent, risk score, assurance details, support action, or immutable event history. Before any real unlink, replacement, or relink, later governance must define immutable event evidence and its purpose-bound access, retention, deletion, and legal-hold treatment. Privileged/support access and mutation remain unauthorized. Provider subjects and durable account IDs are internal sensitive identifiers; they must not become public identifiers, appear in public APIs, or be exposed through enumeration, logs, errors, analytics, or unrelated administration.

No email, phone, username, contact/presentation attribute, credential, token, secret, session, recovery data, or provider payload belongs to the contract. The subject is unbounded PostgreSQL `text` under deterministic `C` collation so governance invents no universal provider maximum. Provider-specific limits, validation/normalization and whether encryption, hashing, or tokenization is justified remain deferred until provider format, lookup, collision, threat, and operational research exists. Physical approval does not authorize schema work, a provider, a runtime reader/writer, real identity data, or personal-data processing.
...
Future session credentials and transports must receive explicit security review for fixation, theft/replay, revocation, compromise containment, privacy, and enumeration resistance. Session authority must be bounded in time and revocable before expiry, with fail-closed handling of invalid or ambiguous account/security state. The credential representation, hashing or storage form, cookies or other transport, lifetime mechanics, renewal/rotation, cryptography, and anomaly-signal implementation remain intentionally undecided under `REG-0034`/`ADR-0015`; any transport-specific CSRF requirements are deferred until that transport is selected.

## Public ALSAMAD Authentication Session Governance Boundary

`REG-0034` / accepted `ADR-0015` define an architecture-only, provider-neutral session boundary following the completed REG-0033 persistence unit. Sessions are server-managed, revocable authority contexts belonging to exactly one durable ALSAMAD account and subordinate to the account's current authentication and security state. They cannot establish or override account or authentication identity authority.

The conceptual lifecycle is `ISSUED`, `ACTIVE`, and `EXPIRED` or `REVOKED`; `INVALID` is an evaluation outcome, not a required persisted state. Every session has bounded authority in time. Independent client/device security contexts may have independently revocable sessions, and account-wide invalidation must be possible. Required conceptual invalidation triggers include current-session logout, account-wide logout/revoke-all, account disablement or closure, security-sensitive authentication-state change, and compromise response. Ordinary profile, content, or preference changes do not by themselves invalidate sessions.

Security invariants require fixation resistance, high-consequence theft/replay containment, fail-closed handling of ambiguous authority, revocation before expiry, concurrent-session isolation, disablement/closure enforcement, authentication-state and privilege-change review, privacy-preserving logout and account-wide logout behavior, and resistance to session enumeration and identifier leakage. Client/device context is a security/session context only and conveys no physical-device identity, durable ownership, trusted-device status, assurance, reputation, personhood, household, Talibeen, or Editorial identity.

No session table, columns, indexes, migration, persistence mechanism, token or cookie representation, transport, endpoint/header, TTL value, refresh mechanism, cryptographic algorithm, storage/cache, provider behavior, or runtime flow is frozen here. Transport-specific CSRF requirements remain deferred until a future transport/cookie model is separately governed. No session rows, cookies, tokens, login/signup, provider integration, API/runtime resolution, Recovery, support/admin mutation, real identity data, personal-data processing, or production activation is authorized.

Recovery remains downstream and separately governed. Immutable audit-event governance remains separately required before real identity lifecycle mutation where existing authority requires it; `created_at`/`updated_at`-style current-state metadata is not audit history. Retention, erasure, legal-hold, backup, and privacy rules remain separately governed before applicable real session or identity processing. No hidden indefinite retention or post-erasure reassignment is authorized. Completion of this architecture unit creates no successor implementation authority.

## Public ALSAMAD Authentication Session Physical Contract Boundary

`REG-0035` / accepted `ADR-0016` approve only a provider-neutral, transport-neutral PostgreSQL physical contract subordinate to `REG-0034`/`ADR-0015`. PostgreSQL is authoritative for persisted session state. The future `user_sessions` entity has exactly five fields: immutable application-generated UUIDv7 record `id`; immutable restrictive ownership FK `user_id` to `users.id`; immutable explicit `created_at`; immutable explicit `expires_at`; and nullable one-way `revoked_at`. Approval creates no table, row, credential, runtime, provider, API, real-data, support/admin, Recovery, production, or successor authority.

Session-record identity and credential identity are separate. `user_sessions.id` is opaque record identity and must not be treated as a bearer credential or secret. No raw bearer/session secret, credential/token/hash, refresh or lookup representation, rotation lineage, provider token, or provider payload is stored. JWT versus opaque credentials, payload, hashing, encryption, credential lookup, refresh, rotation, and reuse detection remain later governance.

Physical bounded-time evidence requires `expires_at > created_at`; no numeric TTL or lifetime policy is selected. Row revocation is represented only by `revoked_at`, which is initially `NULL`, cannot precede `created_at`, may move only once from `NULL` to a valid timestamp, and cannot be cleared or rewritten. `id`, `user_id`, `created_at`, and `expires_at` are immutable, and no-op lifecycle mutation is rejected. There is no mutable status or `updated_at` field.

Lifecycle remains derived: issuance is successful creation; active authority requires an unrevoked, unexpired row plus a current fail-closed account/authentication/security evaluation; expiry is time at or after `expires_at`; revocation is non-null `revoked_at`; and `INVALID` remains evaluation-only. `SUSPENDED` is absent. Multiple non-unique rows may belong to one account, allowing concurrent independently revocable contexts without device tracking. The non-unique `user_id` index supports locating account rows for future account-wide revocation. Generation/version, locking, transaction isolation, concurrent issuance serialization, and runtime revoke-all behavior remain deferred.

Per-context isolation grants no permission to fingerprint. The physical contract contains no IP, user agent, geolocation, fingerprint, hardware/advertising identifier, device label, trusted-device or ownership assertion, authentication-identity FK, provider/contact metadata, assurance, trust, reputation, personhood, household, Talibeen identity, Editorial identity, revocation reason, audit reference, or cleanup metadata.

Operational session state and immutable security evidence remain separate. Audit-event governance is not a prerequisite to this physical governance decision or to a future separately authorized zero-row runtime-inert implementation. Real issuance requires applicable future logging/audit authority; real revocation requires immutable event governance where existing authority requires it; and support/admin mutation requires separately governed immutable purpose-bound evidence. Numeric retention remains unresolved, and real issuance or processing remains blocked pending applicable purpose, lawful basis, notice, retention, erasure, deletion completion, backups, access/export, correction, transfers/subprocessors, support access, legal holds, and jurisdiction obligations.

Recovery and provider/login/signup behavior remain separate. Token/cookie/header/endpoint/serialization/CSRF/middleware mechanics, credential resolution, session runtime, account-wide algorithms, provider behavior, and cache design are not frozen. Any cache or auxiliary store requires separate governance and cannot replace PostgreSQL as canonical authority. A future empty runtime-inert persistence unit requires its own readiness audit, exact Roadmap crossing, and explicit Owner implementation authorization.
