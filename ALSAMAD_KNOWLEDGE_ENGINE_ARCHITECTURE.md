...
**Governed phase status:** `KE-1` — the schema-free, in-memory entity/identity/relationship-shape and deterministic search-matching unification for Quran and Adhkar described in §16 Phase 1 — is implemented at `e073879` under `REG-0014` and remains runtime-inert. `REG-0022`/`ADR-0009` are the current governing sequencing authority for Phase 2's unchanged later-additive `topics` and `content_topics` package described in §6.2–§6.3 and `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1. Delivery is split into `KE-2A` (`topics`; implementation NOT STARTED) followed by `KE-2B` (`content_topics`; implementation NOT STARTED / BLOCKED pending KE-2A COMPLETE and physical `devotional_items`). Historical/Superseded `ADR-0007` §§1–3 remain binding only through `ADR-0009`'s incorporation by reference. Neither unit is implementation-authorized; each requires its own later Governance Unit 2 crossing, and no migration number is assigned. Phase 3 onward, Duas, and every other module's participation remain proposed only and are not approved for implementation. The overall documentation-only status otherwise continues to govern this document as a whole.
...
The first bullet is the `KE-2A` unit; the second is the later `KE-2B` unit. Both are governance-approved for later, non-Release-1 implementation only, but neither is implementation-authorized: each requires its own later Governance Unit 2 crossing. `KE-2A` implementation is NOT STARTED. `KE-2B` implementation is NOT STARTED / BLOCKED pending KE-2A COMPLETE and physical `devotional_items`. Each unit is separately atomic and receives its migration number mechanically only at implementation time; no migration number is assigned here. Exact executable schema is authoritative in `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1. The split does not alter the frozen 30-table Release-1 catalog, the substantive model, or any existing canonical owner. The third bullet and every later layer remain unauthorized.
...
| 2       | **Governance approved; neither unit implementation-authorized (`REG-0022`, `ADR-0009`).** `KE-2A`: controlled topic vocabulary, implementation NOT STARTED. Then `KE-2B`: content-to-topic assignments for Quran ayahs and authenticated Adhkar items only, implementation NOT STARTED / BLOCKED pending KE-2A COMPLETE and physical `devotional_items`. Historical/Superseded `ADR-0007` §§1–3 bind only through ADR-0009 incorporation. Draft may precede owner eligibility, but approval and every later read fail closed against current canonical-owner publication/verification eligibility; both units remain later-additive/non-Release-1, separately atomic, runtime-inert, seed-free, separately gated by a later Governance Unit 2 crossing, and unnumbered until implementation time | 2 total (`topics`, then `content_topics`)                 |
