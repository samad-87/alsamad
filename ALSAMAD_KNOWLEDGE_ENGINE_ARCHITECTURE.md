...
**Governed phase status:** `KE-1` — the schema-free, in-memory entity/identity/relationship-shape and deterministic search-matching unification for Quran and Adhkar described in §16 Phase 1 — is implemented at `e073879` under `REG-0014`. `KE-2` — exactly the later additive `topics` and `content_topics` package for Quran ayahs and authenticated Adhkar items described in §6.2–§6.3, `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1, `ADR-0007`, and `REG-0015` — is governance-approved but implementation is NOT STARTED. Phase 3 onward, Duas, and every other module's participation remain proposed only and are not approved for implementation. The overall documentation-only status otherwise continues to govern this document as a whole.
...
Following the Schema Minimalism Principle (`ALSAMAD_DATABASE_ARCHITECTURE.md` §2.1), `REG-0015`/`ADR-0007` govern Phase 2 as exactly:
...
The first two bullets are governance-approved for a later, non-Release-1 implementation only; they do not authorize a migration or runtime use. Exact executable schema is authoritative in `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1. They do not alter the frozen 30-table Release-1 catalog or any existing canonical owner. The third bullet and every later layer remain unauthorized.
...
| 2       | **Governance approved; implementation NOT STARTED (`REG-0015`, `ADR-0007`).** Controlled topic vocabulary + content-to-topic assignments for Quran ayahs and authenticated Adhkar items only; draft may precede owner eligibility, but approval and every later read fail closed against current canonical-owner publication/verification eligibility; later additive/non-Release-1, runtime-inert, and seed-free | 2 (`topics`, `content_topics`)                            |
