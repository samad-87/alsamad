...
**Status:** Proposed architecture review; documentation only except for separately governed KE-1 and KE-2A units identified below. No broader implementation is approved.

**Governed phase status:** `KE-1` — the schema-free, in-memory entity/identity/relationship-shape and deterministic search-matching unification for Quran and Adhkar described in §16 Phase 1 — is implemented at `e073879` under `REG-0014` and remains runtime-inert. `REG-0022`/`ADR-0009` are the current governing sequencing authority for Phase 2's unchanged later-additive `topics` and `content_topics` package described in §6.2–§6.3 and `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1. Delivery is split into `KE-2A` (`topics`; COMPLETE at `c1757790f579fd34838d9136c9420aa8168e9a2a` under implemented `REG-0024`/supplemental `ADR-0010`, migration `0012`) followed by `KE-2B` (`content_topics`; implementation NOT STARTED / BLOCKED pending physical `devotional_items`, and not implementation-authorized). `REG-0023` is historical/Superseded. Historical/Superseded `ADR-0007` §§1–3 remain binding only through `ADR-0009`'s incorporation by reference. KE-2B still requires its own later Governance Unit 2 crossing and remains unnumbered until implementation. Phase 3 onward, Duas, and every other module's participation remain proposed only and are not approved for implementation. The overall documentation-only status otherwise continues to govern this document as a whole.
...
The first bullet is the `KE-2A` unit; the second is the later `KE-2B` unit. `REG-0024`/supplemental `ADR-0010` supersede historical `REG-0023`; KE-2A is COMPLETE at commit `c1757790f579fd34838d9136c9420aa8168e9a2a` through separately atomic migration `0012`. `KE-2B` remains not implementation-authorized, requires its own later crossing, and is NOT STARTED / BLOCKED pending physical `devotional_items`; its KE-2A COMPLETE prerequisite is now satisfied. KE-2B remains separately atomic and receives its migration number mechanically only at its future authorized implementation time. Exact executable schema is authoritative in `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1. The split does not alter the frozen 30-table Release-1 catalog, the substantive model, or any existing canonical owner. KE-2A remains runtime-inert and seed-free; the third bullet and every later layer remain unauthorized.
...
| 2       | **Governance approved under `REG-0022`/`ADR-0009`; KE-2A implemented under `REG-0024`/supplemental `ADR-0010` (`REG-0023` historical/Superseded).** `KE-2A`: controlled topic vocabulary, COMPLETE at commit `c1757790f579fd34838d9136c9420aa8168e9a2a` through separately atomic migration `0012`, runtime-inert and seed-free. Then `KE-2B`: content-to-topic assignments for Quran ayahs and authenticated Adhkar items only, not implementation-authorized and implementation NOT STARTED / BLOCKED pending physical `devotional_items`; its KE-2A COMPLETE prerequisite is satisfied. Historical/Superseded `ADR-0007` §§1–3 bind only through ADR-0009 incorporation. Draft may precede owner eligibility, but approval and every later read fail closed against current canonical-owner publication/verification eligibility. KE-2B remains later-additive/non-Release-1, separately atomic, seed-free, unnumbered until implementation time, and still requires its own later Governance Unit 2 crossing | 2 total (`topics`, then `content_topics`)                 |
