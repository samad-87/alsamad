...
3. **[A/B split]** For KE-2A, real PostgreSQL verifies `topics` UUIDv7 IDs, immutable unique lowercase canonical keys, locale-key/name JSON integrity, status checks, timestamps, `locales`/`editorial_users` FKs, and topic-local deletion restrictions that require no `content_topics` table. For KE-2B, real PostgreSQL verifies `content_topics` UUIDv7 IDs, review-state checks, timestamps, all assignment FKs — including `topic_id`, `quran_ayah_id`, and `devotional_item_id` — the exactly-one-endpoint constraint, and all assignment/cross-table deletion restrictions.
...
11. **[A+B independent]** Targeted tests for the applicable unit and all KE-1 tests pass. KE-2B may re-run KE-2A tests as cumulative regression evidence; KE-2A does not run or depend on KE-2B schema/tests.
...
16. **[A+B independent]** The staged set for each separately authorized unit equals that unit's exact boundary above with its mechanically assigned migration filename, and the complete staged diff contains no runtime, seed, provider, credential, M5 Gate 4/5, Duas, or later-phase work.
