...
**Governance status.** `REG-0028` and accepted `ADR-0011` promote Public ALSAMAD Identity/Account from Prepared/inactive into an Expanded V1 prerequisite architecture track. `REG-0029` and accepted `ADR-0012` approve the minimal durable account-root physical contract. `REG-0030` authorizes one future, separate runtime-inert implementation unit under the exact contract below. `PUBLIC ALSAMAD IDENTITY ARCHITECTURE = OPEN / APPROVED`; `DURABLE ACCOUNT ROOT PHYSICAL CONTRACT = APPROVED`; `PUBLIC ALSAMAD RUNTIME-INERT DURABLE ACCOUNT ROOT PERSISTENCE = AUTHORIZED / NOT STARTED`. Broader `PUBLIC ALSAMAD IDENTITY IMPLEMENTATION = BLOCKED / NOT AUTHORIZED` except for that explicit unit. No implementation evidence or acceptance PASS exists.
...
**Release status.** Architecture track OPEN / APPROVED under `REG-0028`/`ADR-0011`; durable account-root physical contract APPROVED under `REG-0029`/`ADR-0012`; the exact runtime-inert root unit below is `AUTHORIZED / NOT STARTED` under `REG-0030`. Broader Public Identity implementation remains `BLOCKED / NOT AUTHORIZED`. No implementation evidence or acceptance PASS exists. This track authorizes no Talibeen unit and changes no M5, M6, KE, Quran, devotional, or Core Release 1 status.

### Authorized exact unit — PUBLIC ALSAMAD RUNTIME-INERT DURABLE ACCOUNT ROOT PERSISTENCE

**Authority and objective.** `REG-0030` authorizes one future, separate implementation commit that physically represents only the approved provider-neutral `users` root while keeping it empty, runtime-inert, and inaccessible. `REG-0030` remains `DECIDED`; `REG-0029` remains the `DECIDED` physical-contract decision; `ADR-0012` remains Accepted and unchanged. This governance write is not implementation evidence.

**Exact future implementation boundary.** A later implementation may touch only:

```
src/db/schema.ts
drizzle/<mechanically-assigned-next-number>_public_identity_account_root.sql
drizzle/meta/_journal.json
scripts/db-verify.mjs
```

The migration number is recomputed from the authoritative forward-only sequence when implementation begins. `0013` is only the current audit's observed likely number and is not reserved or permanently pre-claimed. The suffix is exactly `public_identity_account_root`. The journal change is exactly one append using the next index, version `7`, a monotonic `when`, the matching migration tag, and `breakpoints: true`; prior entries and migrations remain unchanged. No snapshot, standalone test, dependency, runtime, API, UI, auth/provider, or Talibeen file is authorized. If tooling requires another file or unrelated generated change, implementation stops for governance review.
