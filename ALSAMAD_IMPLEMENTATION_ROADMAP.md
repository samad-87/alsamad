...
**Status and dependency truth.** KE-1 remains COMPLETE. KE-2 governance remains COMPLETE; KE-2A implementation remains NOT STARTED, and KE-2B implementation remains NOT STARTED / BLOCKED. This prerequisite is now implemented at commit `a2604ca` and removes the `editorial_users` blocker for both units; `devotional_items` remains a hard independent blocker only for KE-2B and does not block KE-2A. Neither unit is implementation-authorized, and each requires its own later Governance Unit 2 crossing. M5 Gate 3 remains PARTIAL; M5 Provider Import Dry Run Verified and M5 Quran Import Activated remain NOT PASS; M6 remains BLOCKED; Duas governance is otherwise unchanged by this prerequisite (`REG-0019`'s separate, narrow M6.0-only M5-dependency exception is unrelated to this entry).
...
**Status.** `M7.0-track` gate structure is recorded. `REG-0014` authorized exactly KE-1, which is now COMPLETE at `e073879`; it remains runtime-inert. `REG-0022`/`ADR-0009` are the current governing sequencing authority for KE-2's unchanged later-additive two-table contract: `KE-2A` implementation is NOT STARTED, and `KE-2B` implementation is NOT STARTED / BLOCKED. `ADR-0007` is historical/Superseded; its §§1–3 remain binding only through `ADR-0009`'s incorporation by reference. Phase 3 onward, Duas, collections/references, generic persisted knowledge edges, runtime UI/search wiring, and AI remain NOT AUTHORIZED. No prototype outside an explicit gate boundary carries authority merely by existing. `M5 Gate 3` remains `PARTIAL`, M5 Gates 4–7 remain `NOT PASS`, `M5 Provider Import Dry Run Verified` and `M5 Quran Import Activated` remain `NOT PASS`, `M6.0` remains `COMPLETE`, and `M6.1`/`M6.2` remain `BLOCKED`. `REG-0019`, `REG-0020`, Sakīnah, Editorial Identity Foundation, and Duas semantics and statuses are unchanged.

#### M7.0-track / KE-2A — Topics Foundation

**Governance status.** `REG-0022`/`ADR-0009` govern the delivery split and incorporate the unchanged `topics` physical specification from historical/Superseded `ADR-0007` §2. They do **not** authorize implementation. `KE-2A` implementation is **NOT STARTED** and requires its own separate, later Governance Unit 2 Registry crossing before migration execution, schema/code implementation, staging, commit, push, seed data, or runtime wiring.
