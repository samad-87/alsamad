...
Left unresolved until Quran.Foundation replies: permanent storage rights beyond the seven-day default (per `REG-0020`, now resolved for unmodified Arabic Quran text specifically; still unresolved for every other resource — translations, tafsir, audio, or any other class); exact attribution wording; commercial-use rights; translation-specific licenses; audio usage rights; Search API production access; exact quotas; long-term caching; offline use; redistribution rights; provider SLA; deletion/exit rights; and the approved legally independent fallback source. None of these unresolved items blocks this architecture contract; each blocks only the acceptance gate that depends on it.
...
**Dependency position.** Independent of M6.1; requires no database schema. May proceed in parallel with, before, or after M6.1. Required, together with M6.1, before M6.2. **Exempted from the phase-level M5 production-activation dependency above** per `REG-0019`'s narrow, M6.0-only Decision Registry exception; `M6.1` and `M6.2` remain fully subject to it. This exemption grants no implementation authorization by itself and does not adopt the existing working-tree Duas prototype; see `REG-0019` for the full exclusion list.
...
**Governance status.** `REG-0016` and `ADR-0008` approve the architecture and exact physical contract for the independently sequenced Editorial Identity Foundation. Editorial Identity Foundation is **IMPLEMENTED** at commit `a2604ca`, which added exactly the `editorial_users` table via `drizzle/0011_editorial_identity_foundation.sql` within the exact file boundary below. This prerequisite is not Phase 7 implementation and does not itself authorize any workflow or runtime surface.
...
**Status and dependency truth.** KE-1 remains COMPLETE. KE-2 governance remains COMPLETE; REG-0024/ADR-0010 supersede REG-0023's incomplete crossing, and KE-2A is COMPLETE at commit `c1757790f579fd34838d9136c9420aa8168e9a2a`; KE-2B implementation remains NOT STARTED / BLOCKED and still requires its own later Governance Unit 2 crossing. Editorial Identity Foundation is implemented at commit `a2604ca` and removed the `editorial_users` blocker for both units; `devotional_items` remains a hard independent blocker only for KE-2B. M5 Gate 3 remains PARTIAL; M5 Provider Import Dry Run Verified and M5 Quran Import Activated remain NOT PASS; M6 remains BLOCKED; Duas governance is otherwise unchanged by these facts (`REG-0019`'s separate, narrow M6.0-only M5-dependency exception is unrelated to this entry).

**Phase 7 reconciliation.** Editorial Identity Foundation governance is approved and Editorial Identity Foundation is implemented at commit `a2604ca`, satisfying only the `editorial_users` prerequisite. `editorial_role_grants`, `review_records`, authentication/provider linking, MFA/passkeys/sessions/recovery, Admin API/UI, editorial workflows, and Phase 7 overall remain NOT STARTED / NOT COMPLETE, and this correction does not itself authorize any of them. Future Phase 7 must reuse immutable `editorial_users.id` and may not replace it with a provider or public-account identity.

### Knowledge Engine governance track (M7.0-track)

This section formally opens the governance track already forward-referenced twice in this document (Phase 6's excluded-scope line: _"`ALSAMAD_KNOWLEDGE_ENGINE_ARCHITECTURE.md` and any Knowledge Engine implementation are M7.0-track and are explicitly not authorized by this contract"_; and the M5 acceptance record's exclusion of _"M7 / Knowledge Engine work"_) and in `ALSAMAD_KNOWLEDGE_ENGINE_ARCHITECTURE.md`'s own header (_"Milestone: M7.0 — Knowledge Engine Architecture"_). It records gate structure and each unit's explicit authorization status. It does not itself implement, migrate, commit, or stage code.
...
**M7.0 — Knowledge Engine Architecture.** Already exists and is complete as a documentation artifact: `ALSAMAD_KNOWLEDGE_ENGINE_ARCHITECTURE.md`, proposing a relationship/discovery layer over existing content modules (nodes referencing existing canonical identities, typed/weighted relationship edges, a layered topic/entity taxonomy, search as a read-only consumer, and mechanically enforced Authentic/Editorial separation). The architecture as a whole remains documentation-only and unapproved for broader implementation; only the separately governed KE-1 and KE-2A units recorded below are implemented. This Roadmap entry creates no additional authority.

**Prototype/evidence, not authority.** The remaining untracked `src/lib/knowledge/collections.ts`, `src/lib/knowledge/references.ts`, and `src/lib/knowledge/adapters/duas.ts` files are quarantined prototype evidence only. KE-2A's committed `src/lib/knowledge/topics.ts` is authoritative solely because commit `c1757790f579fd34838d9136c9420aa8168e9a2a` satisfied the governed KE-2A boundary and acceptance contract; no authority transfers to any other prototype or later Knowledge Engine phase.

**Implementation requires Governance Unit 2.** No Knowledge Engine implementation is authorized until a separate Decision Registry entry (`M7.0-track` Governance Unit 2) explicitly records: (a) which phase of the architecture document's §16 phased adoption path is being authorized; (b) the exact scope and file boundary of that implementation unit; and (c) confirmation of whether that scope stays Registry-only or crosses `ALSAMAD_DECISION_REGISTRY.md` §7's ADR threshold (for example, by introducing a physical table). `REG-0021` was the valid historical Governance Unit 2 crossing for the former, unsplit KE-2 execution unit. `REG-0022` supersedes both that obsolete combined authorization and `REG-0015`'s old atomic architecture decision; `REG-0022`/`ADR-0009` replace the former unit with `KE-2A` and `KE-2B` but authorize neither implementation. Historical/Superseded `REG-0023` supplied KE-2A's first crossing but carried an incomplete boundary/behavior contract. `REG-0024`/`ADR-0010` superseded that crossing, corrected the contract, and authorized the now-complete KE-2A implementation recorded at commit `c1757790f579fd34838d9136c9420aa8168e9a2a`. KE-2B remains NOT STARTED / BLOCKED and requires its own later crossing. `ADR-0007` is historical/Superseded; its §§1–3 remain binding only through `ADR-0009`'s unchanged incorporation by reference.
...
**Status.** `M7.0-track` gate structure is recorded. `REG-0014` authorized exactly KE-1, which is COMPLETE at `e073879` and remains runtime-inert. `REG-0022`/`ADR-0009` remain the current governing architecture and sequencing authority for KE-2's unchanged later-additive two-table contract. `REG-0024`/`ADR-0010` supersede historical REG-0023's incomplete crossing; KE-2A is COMPLETE at commit `c1757790f579fd34838d9136c9420aa8168e9a2a`. KE-2B implementation remains NOT STARTED / BLOCKED and unauthorized pending physical `devotional_items` and its own later crossing. `ADR-0007` is historical/Superseded; its §§1–3 remain binding only through `ADR-0009`'s incorporation by reference. Phase 3 onward, Duas, collections/references, generic persisted knowledge edges, runtime UI/search wiring, and AI remain NOT AUTHORIZED. No remaining prototype byte carries authority merely by existing. `M5 Gate 3` remains `PARTIAL`, M5 Gates 4–7 remain `NOT PASS`, `M5 Provider Import Dry Run Verified` and `M5 Quran Import Activated` remain `NOT PASS`, `M6.0` remains `COMPLETE`, and `M6.1`/`M6.2` remain `BLOCKED`. `REG-0019`, `REG-0020`, Sakīnah, Editorial Identity Foundation, and Duas semantics and statuses are unchanged.

#### M7.0-track / KE-2A — Topics Foundation

**Governance and implementation status.** `REG-0022`/`ADR-0009` govern the delivery split and incorporate the unchanged `topics` physical specification from historical/Superseded `ADR-0007` §2; they did **not** themselves authorize implementation. `REG-0024`/`ADR-0010` superseded historical/Superseded `REG-0023` and supplied KE-2A's corrected Governance Unit 2 crossing. `KE-2A` implementation is **COMPLETE** at commit `c1757790f579fd34838d9136c9420aa8168e9a2a`; completion authorizes no runtime wiring or later Knowledge Engine work.

**Objective and dependencies.** Add exactly the later-additive, non-Release-1 `topics` table defined by `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1.1. It owns a controlled localized topic vocabulary and no canonical religious content. Its dependencies are limited to physically present `locales` and `editorial_users`; it does not depend on `quran_ayahs`, `devotional_items`, `content_items`, `content_topics`, M5, or M6.

**Exact implementation boundary satisfied.** Commit `c1757790f579fd34838d9136c9420aa8168e9a2a` touched exactly:

```
src/db/schema.ts
drizzle/0012_ke2a_topics.sql
drizzle/meta/_journal.json
...
The execution-time sequence mechanically assigned `drizzle/0012_ke2a_topics.sql`; it did not reuse or displace reserved `0010_devotional_content_foundation.sql`. `drizzle/meta/_journal.json` contains exactly one corresponding append, prior entries remain unchanged, and its tag matches `0012_ke2a_topics`. The former untracked `src/lib/knowledge/topics.ts` prototype was fully rewritten under the governed contract rather than adopted. No other prototype file is authorized, and KE-2A implemented no `content_topics` work.

**Implementation constraints.** KE-2A is one atomic, empty, forward-only migration for exactly `topics` and its authorized constraints, triggers, and indexes. Its failure rolls back only that unit. Every prior migration remains byte-unchanged. The repository layer is the only code permitted to access `topics` and must have no network/provider side effect. No runtime/search/UI/provider/AI/seed work, application route, component, runtime composition-root import, related-content surface, editorial/admin UI, or feature flag is authorized.

**Topic lifecycle and timestamp contract.** Supplemental `ADR-0010` permits only `draft → approved`, `draft → retired`, and `approved → retired`. Retirement is terminal; every reverse/reactivation transition and `retired → retired` are forbidden. Retirement preserves existing approval evidence and never fabricates it. `created_at` and initial `updated_at` are database-generated; `created_at` is immutable. Each actual localized-name replacement, approval, or retirement strictly advances `updated_at` using the database-owned event expression `GREATEST(clock_timestamp(), OLD.updated_at + interval '1 microsecond')`. Approval assigns that same event time to `approved_at` and `updated_at`; retirement advances only `updated_at`. No-op and application-supplied lifecycle-timestamp mutations are rejected. Canonical-key correction is one atomic replacement: retire the old draft/approved topic and create a new UUIDv7 draft with its own creator, fresh creation/update timestamps, and null approval evidence.

**Minimum repository and error contract.** The persisted model is `TopicRecord`, distinct from KE-1's presentation-only `KnowledgeTopic`; `src/lib/knowledge/types.ts` remains untouched. The repository provides only create-draft, read-by-ID returning any-state record or absence, complete localized-name replacement for draft/approved on actual change, draft approval, draft/approved retirement, and atomic canonical-key replacement. Every mutation verifies its required `editorial_users` actor is active inside the same repository-owned `READ COMMITTED` transaction; lifecycle writes lock the topic row before state validation. Read-by-ID requires no actor or advisory lock. Generic CRUD, arbitrary patching, identity/key/creator/creation-time mutation, ordinary hard delete, generic status setting, retired resurrection, list/search/runtime APIs, and raw prototype adoption are forbidden. One KE-2A-specific typed error contract exposes only `validation`, `not_found`, `invalid_transition`, `canonical_key_conflict`, `inactive_editorial_actor`, and `database_invariant`; read absence returns absence, and deterministically mappable raw database errors do not leak.

#### M7.0-track / KE-2B — Content Topic Assignments

**Governance status.** `REG-0022`/`ADR-0009` govern the delivery split and incorporate the unchanged `content_topics` physical specification from historical/Superseded `ADR-0007` §3. They do **not** authorize implementation. `KE-2B` implementation is **NOT STARTED / BLOCKED** and requires its own separate, later Governance Unit 2 Registry crossing.

**Objective and dependencies.** Add exactly the later-additive, non-Release-1 `content_topics` table defined by `ALSAMAD_DATABASE_ARCHITECTURE.md` §10.1.2. It owns reviewed assignments to either `quran_ayahs.id` or authenticated Adhkar-only `devotional_items.id`, and no canonical religious content. Execution requires `KE-2A` COMPLETE, `quran_ayahs`, `devotional_items`, `content_items`, and `editorial_users` physically present, with their governing migrations/gates green. `devotional_items` does not yet exist, so KE-2B remains BLOCKED. Draft assignments require canonical endpoints but may precede publication eligibility. Approval and every future read require current canonical-owner eligibility: published Quran structure plus the matching active Arabic release's full §5.3.12 live chain and referenced-ayah text, or an Adhkar `dhikr` with a published `source_verified` revision. KE-1 runtime wiring is not a dependency.

**Exact future implementation boundary.** A future, separately authorized KE-2B execution may touch only:

```
src/db/schema.ts
drizzle/<next-authorized-forward-number>_ke2b_content_topics.sql
src/lib/knowledge/topic-repository.ts
tests/knowledge-topic-layer.test.mjs
scripts/db-verify.mjs
```

The KE-2B migration number is assigned mechanically only at implementation time from the then-authoritative forward-only sequence; it is not assigned, frozen, or pre-claimed here and may not reuse or displace `0010_devotional_content_foundation.sql`. KE-2B is a separately atomic, empty, forward-only migration for exactly `content_topics` and its authorized constraints, triggers, and indexes. Its failure rolls back only that unit and does not reopen or roll back KE-2A. No runtime/search/UI/provider/AI/seed work is authorized.

**Split acceptance contract.** A KE-2A or KE-2B implementation commit may be made only after that unit's own later Governance Unit 2 crossing and only when every requirement assigned to that unit below passes. The `[A]`, `[B]`, `[A/B split]`, and `[A+B independent]` labels preserve the complete original 16-item KE-2 contract across the split. `[A/B split]` assigns the separately stated topic-local clause to KE-2A and the separately stated assignment/cross-table clause to KE-2B. `[A+B independent]` requires each unit to satisfy the same regression, tooling, boundary, or migration-quality obligation independently within its own schema and file boundary; it does not make either unit depend on the other's schema or form one joint migration or rollback unit.

1. **[A/B split]** KE-2A adds exactly `topics`; KE-2B adds exactly `content_topics`. Each table remains classified outside the historical Release-1 30-table catalog.
2. **[A+B independent]** Prior migrations are byte-unchanged, and each unit's mechanically numbered migration is separately forward-only, atomic, and empty.
3. **[A/B split]** For KE-2A, real PostgreSQL verifies `topics` UUIDv7 IDs, immutable unique lowercase canonical keys, locale-key/name JSON integrity, `locales`/`editorial_users` FKs, active-actor enforcement, immutable governed fields, the exact `draft → approved`, `draft → retired`, and `approved → retired` graph, terminal retirement and forbidden reactivation, approval-evidence creation/preservation, database-owned approval time, strict `updated_at` advancement using the exact governed expression, no-op/timestamp-fabrication rejection, atomic canonical replacement, and topic-local deletion restrictions that require no `content_topics` table. For KE-2B, real PostgreSQL verifies `content_topics` UUIDv7 IDs, review-state checks, timestamps, all assignment FKs — including `topic_id`, `quran_ayah_id`, and `devotional_item_id` — the exactly-one-endpoint constraint, and all assignment/cross-table deletion restrictions.
4. **[B]** Exact partial unique indexes `WHERE quran_ayah_id IS NOT NULL AND review_state <> 'rejected'` and `WHERE devotional_item_id IS NOT NULL AND review_state <> 'rejected'` enforce at most one current assignment per pair; rejected rows are terminal immutable history, and a new-identity replacement for the same pair is accepted only when no other current row exists.
5. **[B]** Invalid/missing topics, Quran ayahs, Devotional items, non-dhikr Devotional content, both endpoints, and neither endpoint are rejected; draft may precede publication eligibility, but approval fails unless the Quran ayah/surah are published and the full current Arabic release eligibility chain passes, or the Adhkar item has a published `source_verified` revision.
6. **[B]** Advisory weight accepts only `0.000` through `1.000` and cannot affect authenticity, verification, or publication.
7. **[B]** Curator/reviewer integrity, reviewer separation for approval, and approved/rejected evidence pairing are enforced; the `content_topics` `BEFORE UPDATE` guard rejects every update to an already-rejected row, including state, weight, topic/endpoints, curator/reviewer, notes/evidence, timestamps, and all other columns.
8. **[A/B split]** Synthetic fixtures are non-religious, transaction-scoped, and rolled back; KE-2A seeds zero topic rows, and KE-2B seeds zero religious-content assignment rows.
9. **[A+B independent]** All committed files in each unit contain zero Duas dependency/reference and no collection/reference/generic-edge implementation.
10. **[A+B independent]** `src/app/**`, `src/components/**`, and runtime scripts contain zero import of the applicable unit; absence remains an honest empty/fail-closed state.
11. **[A+B independent]** Targeted tests for the applicable unit and all KE-1 tests pass. KE-2A targeted tests cover every governed minimum repository operation and the six stable error categories without changing KE-1's presentation-only `KnowledgeTopic`. KE-2B may re-run KE-2A tests as cumulative regression evidence; KE-2A does not run or depend on KE-2B schema/tests.
12. **[A+B independent]** `npm run typecheck`, `npm run lint`, `npm test`, targeted Prettier check, production build, and `git diff --check` pass independently for each unit.
13. **[A/B split]** For KE-2A, real PostgreSQL concurrency tests prove the topic-local `topics`/`locales` protocol: both mutation paths acquire the one fixed transaction-scoped lock `pg_advisory_xact_lock(hashtextextended('alsamad:ke2:locale-integrity', 0))` at required `READ COMMITTED`; concurrent multi-row topic writes using locale sets `{a,b}` and `{b,a}` do not deadlock; topic insert/update versus locale deletion cannot commit a dangling locale-key reference in either transaction ordering; referenced locale deletion fails; lawful unreferenced locale deletion succeeds; unsupported isolation levels fail closed; the lock is automatically released at commit/rollback; and normal read-only topic queries acquire no advisory lock. For KE-2B, real PostgreSQL cross-table tests prove `content_topics` references and deletion races: topic deletion while referenced by `content_topics` fails; lawful deletion of a topic with no assignments succeeds subject to the topic-local lifecycle; concurrent `content_topics` assignment versus topic deletion cannot both commit a dangling `topic_id` reference in either transaction ordering; and every cross-table test requiring `content_topics` physically present runs only in KE-2B. KE-2B may re-run KE-2A's topic/locale concurrency suite but does not weaken or replace its evidence.
14. **[B]** A future read-model test proves topic/assignment approval alone is insufficient: the same canonical-owner eligibility predicate is re-derived at read time, and Quran withdrawal/unpublication/release or rights ineligibility and Adhkar withdrawal/supersession/verification loss fail closed without mutating the assignment or blocking the owner transition.
15. **[B]** Rejected-assignment PostgreSQL tests prove rejected-to-draft, rejected-to-approved, weight changes, reviewer/evidence/notes changes, timestamp changes, and every other update fail; insertion of a new replacement for the same pair succeeds, while the partial indexes prevent two non-rejected rows for that pair.
16. **[A+B independent]** The staged set for each separately authorized unit equals that unit's exact boundary above with its mechanically assigned migration filename, and the complete staged diff contains no runtime, seed, provider, credential, M5 Gate 4/5, Duas, or later-phase work. For KE-2A the exact boundary is seven files, including `drizzle/meta/_journal.json`; its journal diff is exactly one mechanical append, modifies no prior entry, and its tag exactly matches the mechanically assigned migration filename.

**KE-2A Topics Foundation Verified = PASS.** Commit `c1757790f579fd34838d9136c9420aa8168e9a2a` satisfies every KE-2A-owned obligation in items 1, 2, 3, 8, 9, 10, 11, 12, 13, and 16. Its exact seven-file boundary includes mechanically assigned migration `drizzle/0012_ke2a_topics.sql` and the single matching journal append. Verification passed `npm run db:check`, `npm run typecheck`, `npm run lint`, `npm test` (486/486), `npm run build`, targeted formatting, and Git diff checks. Real PostgreSQL evidence covers topic INSERT and localized-name UPDATE versus locale DELETE in both transaction orderings, referenced/unreferenced deletion outcomes, unsupported-isolation failure, `{a,b}`/`{b,a}` no-deadlock behavior, advisory-lock release after commit and rollback, and lock-free ordinary reads. KE-2B-owned items 4–7, 14, and 15 remain unsatisfied and unclaimed.

**Explicit exclusions and protected status truth.** `src/lib/knowledge/collections.ts`, `src/lib/knowledge/references.ts`, `src/lib/knowledge/adapters/duas.ts`, generic persisted `knowledge_edges`, Articles/Guides, Hadith, Talibeen, Duas, entities, topic-to-topic edges, AI suggestions, semantic search, the AI Search Assistant, runtime search expansion, related-content UI, editorial/admin UI, topic/assignment seeds, provider/network/credential work, M5 Gate 4/5, canonical ownership changes, and Phase 3 onward remain NOT AUTHORIZED. Neither unit authorizes KE-3. `M5 Gate 3` remains `PARTIAL`; M5 Gates 4–7 and `M5 Quran Import Activated` remain `NOT PASS`; `M6.0` remains `COMPLETE`; `M6.1`/`M6.2` remain `BLOCKED`; and `REG-0019`, `REG-0020`, Sakīnah, Editorial Identity Foundation, and Duas semantics and statuses remain unchanged.

**Completion evidence.** KE-2A is COMPLETE at commit `c1757790f579fd34838d9136c9420aa8168e9a2a` under `KE-2A Topics Foundation Verified = PASS`. KE-2B remains `NOT STARTED / BLOCKED`, and only a later separately authorized implementation commit satisfying every KE-2B-owned acceptance item may mark KE-2B COMPLETE. Combined KE-2 is not complete.
...
**Governance status.** `ALSAMAD_PRODUCT_ARCHITECTURE_V1.md`'s "Alsamad SEO & Discoverability Architecture" section already settles the architectural *what* for this narrow subset; no new Decision Registry entry is required, consistent with Registry §1 ("the Decision Registry exists to record exactly one thing: the status and outcome of an architecturally relevant decision that is not yet, or was not already, settled by an existing architecture document"). This Roadmap gate supplies only the *when*, per Registry §2 point 4. Implementation is **COMPLETE** at commit `2247950b3137f71087349f47a99f3bfad4eb1816`. `SEO FOUNDATION UNIT-1 ACCEPTANCE = PASS`. Current HEAD retains the accepted implementation, no later commit has modified any Unit-1 implementation path, and no reimplementation is required. `REG-0018` (Sakīnah Visual Foundation Implementation) did not authorize this unit — its own explicit-exclusions clause names "SEO" by name as outside that separate gate.

**Implementation evidence and historical boundary disposition.** The post-implementation historical acceptance audit verified all acceptance items A–K: targeted SEO tests PASS (`34/34`); full suite PASS (`486/486`); typecheck PASS; lint PASS; production build PASS; static generation PASS (`265/265`); `/robots.txt` and `/sitemap.xml` built; targeted Prettier PASS; and `git diff --check` PASS. Historical implementation commit `2247950b3137f71087349f47a99f3bfad4eb1816` touched one path outside the exact Unit-1 boundary, `tests/knowledge-entity-layer.test.mjs`, solely to remove one unused import. The audit classified this as a **HARMLESS ADMINISTRATIVE DEVIATION**: no test body or assertion changed, no Knowledge implementation changed, coverage was not weakened, SEO scope did not expand, and the full 486-test suite passes. No corrective code patch is required. This record acknowledges the deviation without rewriting history or expanding the Unit-1 boundary.
...
**Governance status.** `REG-0028` and accepted `ADR-0011` promote Public ALSAMAD Identity/Account from Prepared/inactive into an Expanded V1 prerequisite architecture track. `REG-0029` and accepted `ADR-0012` approve the minimal durable account-root physical contract, and `REG-0030` records its exact runtime-inert implementation COMPLETE/PASS. `REG-0031`/`ADR-0013` approve authentication-linkage architecture, `REG-0032`/`ADR-0014` separately approve its physical contract, and `REG-0033` records the exact runtime-inert persistence unit as COMPLETE/PASS. `PUBLIC ALSAMAD IDENTITY ARCHITECTURE = OPEN / APPROVED`; `DURABLE ACCOUNT ROOT PHYSICAL CONTRACT = APPROVED`; `PUBLIC ALSAMAD RUNTIME-INERT DURABLE ACCOUNT ROOT PERSISTENCE = COMPLETE`; `Public ALSAMAD Durable Account Root Persistence Verified = PASS`; and `PUBLIC ALSAMAD AUTHENTICATION IDENTITY LINKAGE PHYSICAL CONTRACT = APPROVED`. Broader `PUBLIC ALSAMAD IDENTITY IMPLEMENTATION = BLOCKED / NOT AUTHORIZED` outside these bounded inert units.
...
**First architecture boundary.** Durable account identity; authentication boundary; session boundary; recovery boundary; essential usable/restricted-or-disabled/deletion-in-progress/terminal-completion lifecycle semantics; Editorial Identity separation; future private-module linkage suitability; multi-client reuse; and deletion coordination that prevents blind cascade, orphaning, or undeclared retention. `REG-0029`/`ADR-0012` freeze the `users` root, and `REG-0032`/`ADR-0014` separately freeze only the provider-neutral authentication-linkage physical contract. `REG-0033` records the completed inert persistence unit; all runtime linkage, sessions, recovery persistence, provider choice, terminal disposition policy, real-data processing, and runtime surfaces remain unresolved/deferred or blocked.

**Explicit exclusions.** Except for the exact runtime-inert `users` physical unit below, preferences, saved items, favorites, synchronization, personalization, private routines, notifications, payments, unrelated account features, Talibeen linkage/profile/persistence/discovery/introductions/messaging, real accounts/data, provider/mechanism selection or integration, credentials, session storage, recovery implementation, functional API contract/endpoint, route, UI, Admin/support tooling, runtime wiring, and deployment remain blocked. The approved negative API no-surface contract creates no endpoint or runtime authority. The Core Release 1 guest-first experience and 30-table boundary are unchanged.
...
**Authorization dependency result.** `REG-0029`/`ADR-0012` satisfy the durable-root physical Database contract, minimum inert-root Security/privacy acceptance contract, and API no-surface contract. `REG-0030` records the Owner-reviewed exact unit, scoped threat/data-flow assessment, rollback rules, file/migration/verification boundary, acceptance contract, and accepted completion evidence. Provider/mechanism selection remains unnecessary and deferred for this zero-row inert unit. Real-user activation separately requires the unresolved privacy, deletion/user-rights, provider, operational, and jurisdiction gates. Completion of this exact unit creates no next implementation authority.
...
**Release status.** Architecture track OPEN / APPROVED under `REG-0028`/`ADR-0011`; durable account-root physical contract APPROVED under `REG-0029`/`ADR-0012`; the exact runtime-inert root unit below is `COMPLETE` under implemented `REG-0030`, with `Public ALSAMAD Durable Account Root Persistence Verified = PASS`; authentication-linkage architecture is OPEN / APPROVED under `REG-0031`/`ADR-0013`; its separate physical contract is APPROVED under `REG-0032`/`ADR-0014`; and its exact inert persistence unit is `COMPLETE` under implemented `REG-0033`, with `Public ALSAMAD Authentication Identity Linkage Runtime-Inert Persistence Verified = PASS`. Runtime linkage and broader Public Identity implementation remain `BLOCKED / NOT AUTHORIZED`. This track authorizes no Talibeen unit and changes no M5, M6, KE, Quran, devotional, or Core Release 1 status.

### Completed exact unit — PUBLIC ALSAMAD RUNTIME-INERT DURABLE ACCOUNT ROOT PERSISTENCE

**Authority and objective.** `REG-0030` authorized one separate implementation commit that physically represents only the approved provider-neutral `users` root while keeping it empty, runtime-inert, and inaccessible. The exact unit is COMPLETE at commit `62e6df28c2f66f93b6a2633a9d23de026c4d4085`; `REG-0030` is `IMPLEMENTED`; `REG-0029` remains the `DECIDED` physical-contract decision; and `ADR-0012` remains Accepted and unchanged. Completion records evidence only and grants no new implementation authority.

**Exact implementation boundary satisfied.** Commit `62e6df28c2f66f93b6a2633a9d23de026c4d4085` (`feat: implement inert durable account root persistence`) touched exactly:
...
The authoritative forward-only sequence mechanically assigned `0013` with the exact suffix `public_identity_account_root`. The journal change is exactly one append using index `12`, version `7`, a monotonic `when`, the matching migration tag, and `breakpoints: true`; prior entries and migrations remain unchanged. No snapshot, standalone test, dependency, runtime, API, UI, auth/provider, or Talibeen file was added. No next file or implementation unit is authorized.

**Exact physical unit.** Add exactly `users` with exactly four columns: `id uuid PRIMARY KEY NOT NULL` with no database default; `status varchar(24) NOT NULL DEFAULT 'active'`; `created_at timestamptz NOT NULL DEFAULT current_timestamp`; and `updated_at timestamptz NOT NULL DEFAULT current_timestamp`. `id` is application-generated UUIDv7, opaque, immutable, ALSAMAD-owned, provider/client/module independent, not derived from contact/provider data, and never reused. The existing `uuid` dependency and unchanged `src/db/ids.ts` already supply UUIDv7; no new dependency, helper, writer, or runtime generation path is authorized. The migration may add the established `ck_users__id_uuidv7` UUIDv7/RFC-variant check. The primary-key index is the only index.

**Lifecycle integrity.** `ck_users__status` permits exactly `active`, `disabled`, `deletion_pending`, and `deleted`. The governed trigger is exactly `trg_users__lifecycle_integrity`; its supporting PostgreSQL function name is a local implementation detail. It rejects `id` or `created_at` mutation, timestamp-only and no-op updates, status changes without an explicitly supplied strictly later `updated_at`, entry to `deleted` except from `deletion_pending`, and every transition out of `deleted`. `DEFAULT current_timestamp` is insertion-only; no automatic update-on-write mechanism is authorized.

**Migration, data, and surface boundary.** The one isolated additive forward-only migration may contain only `users`, the four approved columns/defaults, UUIDv7/RFC and status checks, lifecycle function/trigger, and mechanically necessary SQL. It creates no seed, backfill, real or retained fixture row, FK, cascade, dependent table, auth link, session, recovery structure, or other table. The root contains no contact, username, display/profile, provider, credential, preference, saved-item, marketing, analytics, Editorial, Talibeen, or module-specific data. It has no reader/writer, repository/service, route/handler, UI, server action, background consumer, auth/provider integration, account-creation path, composition-root consumer, or Talibeen linkage. It remains absent from REST, GraphQL, RPC, request/response contracts, public identifiers, serialization, and every API/runtime surface.

**Scoped threat/data-flow assessment.** The only data flow is reviewed migration tooling applying structure to PostgreSQL and transaction-scoped synthetic verification that rolls back and leaves zero rows. The unit has no credential, contact, provider, session, recovery, module/Talibeen, runtime, or public/API surface. Acceptance must close the relevant risks: drift from `ADR-0012`, incorrect UUID semantics, lifecycle bypass, accidental runtime exposure, migration/journal corruption, and accidental sensitive fields or relationships. This authorization is not approval for real-user processing.

**Acceptance contract.** Real PostgreSQL and repository verification must prove: `users` exists with exactly the four governed fields, types, nullability, and defaults; UUIDv7/RFC acceptance and invalid version/variant rejection; no database UUID default; duplicate-ID rejection; status default/check and invalid-status rejection; immutable `id` and `created_at`; rejection of timestamp-only, no-op, unchanged/earlier-`updated_at` status mutations; success with an explicitly later `updated_at`; `deletion_pending → deleted`; rejection of direct invalid entry to `deleted` and every transition out of it; primary-key-only indexing; no prohibited field/FK/cascade; zero seed/backfill/retained fixture rows; exact migration and one-entry journal boundary; prior-migration byte integrity; and zero runtime/API/Talibeen integration. Synthetic fixtures must be transaction-scoped and rolled back. Runtime scanning must cover relevant `src/app`, `src/components`, `src/lib`, server-action, non-verifier-script, and composition-root locations while excluding legitimate schema/verifier references rather than relying on brittle global text matching.

Required commands are `npm run db:check`, `npm run typecheck`, `npm run lint`, `npm run format:check`, `npm test`, `npm run build`, and `git diff --check`. Exact implementation-file and staged-file review, zero-row proof, zero-runtime proof, API no-surface proof, Talibeen non-expansion proof, prior-migration integrity, exactly-one-journal-append review, and rollback evidence are also mandatory. Compilation alone cannot pass the unit.

**Rollback.** Migration execution failure rolls back atomically through the existing migration transaction. Before database application, rollback reverts the exact implementation commit. After application, and only while `users` remains verified empty and unconsumed, removal requires a separately reviewed forward cleanup migration. No committed migration or journal history may be rewritten and no production user data may be silently dropped. Because the unit is inert, no provider, session, API, route, Talibeen/module, or user-data cleanup is expected.

**Research and exclusions.** No external research is required before this inert unit. Provider/runtime integration later requires separate authentication-security, provider, session, recovery, and operational-support research/governance. Real-user activation later requires privacy/legal, retention/deletion, backup, access/export, audit, transfers/subprocessors, Security, and jurisdiction research/governance. Talibeen production linkage additionally requires identity/age-assurance, privacy, legal, safety, moderation, retention, and jurisdiction governance.

This unit authorizes no provider selection/integration, `user_identities`, `user_sessions`, authentication/login/signup, recovery persistence/runtime, account repository/service/API/UI, real account or personal-data processing, preferences/saved items, Editorial linkage, Talibeen linkage/profile/persistence/discovery/introductions/messaging/moderation/API/UI, notifications, payments, SEO, analytics, AI, background job, deployment, broader Public Identity implementation, or next Talibeen unit.

**Completion evidence.** `Public ALSAMAD Durable Account Root Persistence Verified = PASS`. The unit is COMPLETE at commit `62e6df28c2f66f93b6a2633a9d23de026c4d4085` (`feat: implement inert durable account root persistence`) within the exact four-file boundary above, including migration `0013_public_identity_account_root.sql` and exactly one matching journal append. The exact four-column `users` contract, UUIDv7/RFC-variant checks, lifecycle integrity, primary-key-only/zero-FK state, zero retained rows, migration/journal integrity, zero runtime consumers, API no-surface, and Talibeen isolation are PASS. Verification passed `db:check`, typecheck, lint, targeted implementation formatting, repository tests 497/497, production build with 265 static pages, and Git diff checks. Repository-wide `format:check` retains 41 pre-existing unrelated failures; none of the Prettier-supported implementation files is implicated. Remote proof is `HEAD == origin/main == live remote main == 62e6df28c2f66f93b6a2633a9d23de026c4d4085` with divergence `0 / 0`.

**Status truth.** `PUBLIC ALSAMAD RUNTIME-INERT DURABLE ACCOUNT ROOT PERSISTENCE = COMPLETE`. `REG-0033` records the exact `user_identities` inert persistence unit as COMPLETE/PASS. `user_sessions`, provider selection/linking, authentication runtime, recovery persistence/workflow, preferences, saved items, account/session APIs, runtime readers/writers, account creation, real-user activation, deletion/retention implementation, profile linkage, Talibeen identity linkage, and broader Public Identity implementation remain `BLOCKED / NOT AUTHORIZED`. Core Release 1 remains guest-first and exactly 30 tables; M5 Gate 3 remains PARTIAL; M5 Gates 4–7 and Quran Import Activated remain NOT PASS; M6.1/M6.2 remain BLOCKED; KE-1 remains COMPLETE/runtime-inert; KE-2A remains COMPLETE; KE-2B remains BLOCKED; combined KE-2 remains incomplete; no KE-3 authority exists. `TALIBEEN FOUNDATION = COMPLETE`, `Talibeen Foundation Verified = PASS`, and broader Talibeen remains `BLOCKED / NOT AUTHORIZED` with no next unit.

### Approved architecture unit — PUBLIC ALSAMAD PROVIDER-NEUTRAL AUTHENTICATION IDENTITY LINKAGE GOVERNANCE BOUNDARY

**Governance status.** `REG-0031` and accepted `ADR-0013` establish `PUBLIC ALSAMAD AUTHENTICATION IDENTITY LINKAGE ARCHITECTURE = OPEN / APPROVED`. This architecture decision is complete. The separate physical contract is now APPROVED only under `REG-0032`/`ADR-0014`; `PUBLIC ALSAMAD AUTHENTICATION IDENTITY LINKAGE IMPLEMENTATION = BLOCKED / NOT AUTHORIZED`; provider selection is `NOT STARTED / NOT AUTHORIZED`; and broader `PUBLIC ALSAMAD IDENTITY IMPLEMENTATION = BLOCKED / NOT AUTHORIZED`.

**Decision boundary.** One durable account may eventually link to multiple authentication identities; one authentication identity may resolve to at most one account. Ambiguity, conflict, collision, stale linkage, or prior linkage elsewhere fails closed. Mutable email, phone, or similar contact data never establishes identity equivalence. Future additional linkage requires proof of control of both the existing durable account and new authentication identity. There is no automatic merge, transfer, takeover, or duplicate-account creation. Unlinking must not silently strand access; replacement/relink must not automatically erase necessary security/audit lineage. Authentication assurance conveys access-control evidence only and no trust, reputation, piety, compatibility, Editorial/Talibeen authority, membership tier, payment status, or social standing.

**Threat, privacy, and research boundary.** Conceptual actors are the durable account, authentication identity, future authenticator/provider, future client, and future account-resolution boundary. Threats include duplication, takeover, mutable-contact and provider-subject collisions, ambiguous or stale mapping, unsafe unlink, cross-account transfer, trust/privilege conflation, enumeration, and identifier leakage. No personal-data processing begins. Purpose limitation, minimization, lawful basis, notice, provider metadata, retention, deletion completion, access/export, audit/log and support access, subprocessors, transfers, and jurisdiction obligations remain later gates. No external research is required for this provider-neutral conceptual boundary; it is required before provider selection/integration, real identities, runtime linkage, session/recovery mechanisms, real-user activation, or production processing.

**No implementation authority from this architecture unit.** This architecture unit itself authorizes no physical contract or implementation. The separate `REG-0032`/`ADR-0014` crossing freezes the physical design, and `REG-0033` records completion of only the exact inert implementation boundary. No provider, credential, contact storage, signup/login, API, repository/service, runtime reader/writer, real account/row, session, recovery, preference, saved item, profile, deletion/retention implementation, Editorial linkage, Talibeen linkage, or deployment is authorized.

**Governance sequencing.** Linkage governance precedes session governance, which precedes recovery governance. This is dependency ordering only and authorizes none of their physical or runtime implementation. Sessions and recovery remain future governance; no next runtime unit is authorized.

**Protected status truth.** Durable Account Root remains COMPLETE/PASS and `REG-0030` remains IMPLEMENTED. Core Release 1 remains guest-first and exactly 30 tables; M5 Gate 3 remains PARTIAL; M5 Gates 4–7 and Quran Import Activated remain NOT PASS; M6.1/M6.2 remain BLOCKED; KE-1 remains COMPLETE/runtime-inert; KE-2A remains COMPLETE; KE-2B remains BLOCKED; combined KE-2 remains incomplete; no KE-3 authority exists. Talibeen Foundation remains COMPLETE/PASS and broader Talibeen remains `BLOCKED / NOT AUTHORIZED`.

### Approved physical-contract unit — PUBLIC ALSAMAD PROVIDER-NEUTRAL AUTHENTICATION IDENTITY LINKAGE PHYSICAL CONTRACT

**Governance status.** `REG-0032` and accepted `ADR-0014` establish `PUBLIC ALSAMAD AUTHENTICATION IDENTITY LINKAGE PHYSICAL CONTRACT = APPROVED`. `PUBLIC ALSAMAD AUTHENTICATION IDENTITY LINKAGE ARCHITECTURE = OPEN / APPROVED` remains unchanged under `REG-0031`/`ADR-0013`. `REG-0033` records the exact inert persistence unit as `COMPLETE / VERIFIED PASS`. Runtime linkage, `PROVIDER INTEGRATION`, `SESSION`, `RECOVERY`, `REAL AUTHENTICATION IDENTITY DATA`, and broader `PUBLIC ALSAMAD IDENTITY IMPLEMENTATION` remain `BLOCKED / NOT AUTHORIZED`.

**Persistence decision.** ALSAMAD-owned persistence is justified because replaceable provider/runtime state cannot supply the stable mapping to `users.id`, enforce fail-closed ownership continuity while evidence is retained, or preserve ownership across provider replacement. The approved physical contract is a future `user_identities` table only; this approval creates no table or implementation authority.

**Exact physical contract.** Exactly seven columns: application-generated immutable UUIDv7 `id`; immutable non-null `user_id`; immutable `authenticator_namespace varchar(128)` under case-sensitive `C` collation and checked as an ALSAMAD-controlled lowercase ASCII token; immutable non-empty opaque `subject text` under case-sensitive `C` collation with no universal provider maximum; explicit checked `status varchar(16)` with no default and limited to `active` or `retired`; immutable `created_at timestamptz`; and latest-current-state-only `updated_at timestamptz`. The namespace bound limits only ALSAMAD's internal canonical vocabulary. Provider-specific subject limits, validation/normalization and encryption/hashing/tokenization remain deferred.

**Integrity and indexes.** `user_id` has a non-null, non-deferrable FK to `users.id` with update/delete restriction. Unconditional unique (`authenticator_namespace`, `subject`) coverage applies to retained active and retired rows, closes concurrent duplicate insertion, and prevents cross-account reassignment while that evidence exists. `user_id` is not unique, allowing multiple identities per account. Only the primary-key index, canonical unique index, and one non-unique `user_id` FK/account-disposition index are approved. The future integrity trigger freezes identity/account/canonical-key/creation immutability and permits `updated_at` changes only for real status transitions with a strictly later explicit value.

**Lifecycle and lineage.** `retired` is a retention-bounded inactive linkage record ineligible for future resolution; it preserves ownership continuity only while lawfully retained. Relink may only reactivate that same retained row for the same account and identity under later proof/runtime/audit governance. Replacement creates a distinct canonical row and retires the prior row. Hard deletion is not ordinary unlink. `updated_at` records only the latest current-state transition, not prior transitions or immutable lineage. The table is not an audit-event system; real unlink/replacement/relink remains blocked until later Security/privacy/runtime governance authorizes necessary immutable event evidence, access, retention, deletion, legal hold, proof of control, and unlink safety.

**Erasure and post-erasure boundary.** Retention duration, lawful erasure, deletion completion, legal holds, backups, and jurisdiction rules remain later governance. If lawful erasure or retention expiry deletes a row, this table loses the canonical evidence and cannot enforce post-erasure non-reassignment. Deletion creates no automatic reassignment authority. No permanent denylist/hash/tombstone is approved. Provider-subject recycling and exceptional reassignment remain unsupported pending separate provider research, proof requirements, Security/privacy governance, and immutable audit evidence; without such authority, conflicts fail closed.

**Data minimization and inertness.** The contract contains no email, phone, username, contact/profile field, credential, token, secret, session, recovery data, assurance detail, provider payload, audit event, Editorial, Talibeen, analytics, marketing, or module data. It remains API-inert, runtime-inert, provider-neutral, session-independent, recovery-independent, zero-row, and personal-data-processing-inactive. No public identifier, endpoint, serialization, repository/service, runtime consumer, provider integration, row, or real identity is authorized.

**Research and real-data gates.** Provider subject guarantees, limits, recycling/reassignment behavior, validation/normalization, secrecy representation, encryption/hashing/tokenization, provider metadata, assurance, operational behavior, subprocessors/transfers, and jurisdiction obligations require later research. Lawful basis, notice, purpose confirmation, retention duration, lawful erasure, deletion completion, access/export, correction, backups, audit/support access, and production threat/data-flow approval must pass before real authentication identity processing.

**Subsequent inert-unit crossing.** At the `REG-0032` crossing, an isolated additive zero-row implementation was only conceptually possible and no migration, file boundary, verifier, or implementation unit was authorized. `REG-0033` defined that exact boundary and acceptance contract, which is now complete under the verified implementation commit above. No API/provider/session/recovery integration, row, consumer, or production activation is authorized by this completion; no successor authority is granted.

**Protected status truth.** `REG-0030` remains IMPLEMENTED and Durable Account Root remains COMPLETE/PASS. Product Architecture, Core Release 1 guest-first scope and 30-table count, M5/M6/KE/Quran status, Talibeen Foundation COMPLETE/PASS, and broader Talibeen BLOCKED / NOT AUTHORIZED status are unchanged. Governance sequencing remains linkage governance → session governance → recovery governance; this approval does not open session or recovery work.

### Governance-authorized exact unit — PUBLIC ALSAMAD AUTHENTICATION IDENTITY LINKAGE RUNTIME-INERT PERSISTENCE IMPLEMENTATION

**Governance status.** `REG-0033` is `IMPLEMENTED`. The exact bounded unit is `COMPLETE`, verified at commit `4056876ee738a4a1d58b03ec8081b89393f33051` (`feat: add inert authentication identity persistence`) within the exact four-file boundary below. `REG-0032`/`ADR-0014` remain the unchanged physical authority.

**Dependency chain and objective.** `REG-0028`/`ADR-0011` → `REG-0029`/`ADR-0012` → implemented `REG-0030` → `REG-0031`/`ADR-0013` → `REG-0032`/`ADR-0014` → implemented `REG-0033`. The completed unit implemented only the approved provider-neutral `user_identities` physical contract as an empty, API-inert, runtime-inert, session-independent, recovery-independent persistence structure. No provider/session/recovery governance or external research was required for this empty unit; each remains mandatory before its applicable runtime or real-data use.
...
The migration is the mechanically next sequence after committed `0013_public_identity_account_root.sql`. The journal change is exactly one mechanical append for `0014_public_identity_authentication_linkage`; every prior entry and migration remains unchanged. No standalone test, snapshot, dependency, runtime, API, provider, session, recovery, UI, service, repository, or unrelated file is authorized. A fifth file or material contract change requires implementation to stop and return to governance.

**Exact physical unit.** Create exactly `user_identities` with exactly seven columns and no eighth field: immutable application-generated UUIDv7 `id uuid PRIMARY KEY NOT NULL` with no database default; immutable `user_id uuid NOT NULL`; immutable `authenticator_namespace varchar(128) COLLATE "C" NOT NULL`; immutable non-empty opaque `subject text COLLATE "C" NOT NULL`; explicit `status varchar(16) NOT NULL` with no default and exactly `active` or `retired`; immutable `created_at timestamptz NOT NULL DEFAULT current_timestamp`; and latest-current-state-only `updated_at timestamptz NOT NULL DEFAULT current_timestamp`. No type, nullability, default, collation, comparison, or semantic choice may differ from `ADR-0014`.

**Integrity and indexes.** Require UUIDv7/RFC-variant integrity; the non-null immutable `user_id` FK to `users(id)` as `NOT DEFERRABLE`, `ON UPDATE RESTRICT`, `ON DELETE RESTRICT`; ALSAMAD-controlled namespace grammar `[a-z0-9][a-z0-9._-]{0,127}`; non-empty exactly stored subject; closed explicit `active`/`retired` status; immutable `id`, `user_id`, namespace, subject, and `created_at`; rejection of no-op/timestamp-only updates; and an explicitly supplied strictly later `updated_at` for a real status transition. The exact three indexes are the primary-key index, unconditional retained-row unique index on (`authenticator_namespace`, `subject`), and non-unique `ix_user_identities__user_id`. No other index is authorized.

**Lifecycle, audit, retention, and erasure.** Database capability for `active`/`retired` integrity authorizes no runtime caller or link/unlink/relink/replacement behavior. `created_at` and `updated_at` are not complete immutable audit lineage; `updated_at` records only the latest governed state transition. Real mutation remains blocked pending a separately governed immutable audit-event/Security/runtime contract. Retained-row uniqueness applies only while canonical evidence exists. The empty implementation decides no lawful basis, retention duration, erasure completion, legal hold, backup, user-rights mechanism, provider retention, or post-erasure reassignment. No permanent denylist, hash reservation, tombstone identifier, or indefinite retention is authorized, and deletion grants no reassignment authority.

**Zero-row and inert acceptance.** Immediately after migration and verification, `SELECT count(*) FROM user_identities` must equal `0`. No seed, backfill, retained fixture, imported/provider/synthetic/production identity, or real row may remain. Any synthetic verification data is transaction-scoped and rolled back. Runtime scanning must prove zero consumers in application routes, components, server actions, API handlers, services, repositories, authentication/provider flows, sessions, recovery, Talibeen, Editorial Identity, and other modules. The unit creates no endpoint, serialization, public/staff API, login/signup, identity resolution, provider integration, deployment, or production activation.

**Provider and data minimization.** No Google, Apple, OAuth/OIDC-provider, passkey, password, email, phone, SMS, subject-normalization, subject-recycling, reassignment, assurance, provider-payload, contact/profile, credential, token, secret, session, recovery, audit actor/reason/proof, Editorial, Talibeen, or module field/logic belongs to the unit. Namespace and subject are unused generic storage capability only. Real authentication identity data and personal-data processing remain prohibited.

**Acceptance contract.** Real PostgreSQL and repository verification must prove the exact seven columns/types/nullability/defaults/collations; PK and application-supplied UUIDv7/version/variant rules; FK target/actions/non-deferrability; namespace grammar; subject non-empty and exact comparison; explicit status/no default/closed domain; immutable fields; valid active/retired transitions and rejection of illegal, no-op, timestamp-only, unchanged/earlier timestamp mutations; latest-state-only `updated_at`; unconditional retained-row uniqueness and collision/concurrency failure where practical; exact three indexes; prohibited-field absence; and zero rows. It must also prove the exact migration and one-entry journal append, prior-migration byte integrity, exact four-file boundary, zero runtime consumers, API no-surface, provider/session/recovery absence, Talibeen and Editorial isolation, and rollback readiness.

Required gates are `npm run db:check`, `npm run typecheck`, `npm run lint`, targeted implementation-file formatting, repository tests, `npm run build`, and `git diff --check`, plus exact implementation and staged-file review. Any pre-existing unrelated repository-wide formatting debt must be reported without widening scope. Compilation alone is insufficient.

**Rollback.** Migration failure rolls back atomically. Verification failure prevents acceptance and leaves zero retained rows. Before application, rollback reverts the exact unaccepted implementation change. After application, committed migration history is never rewritten; while the table remains proven empty and unconsumed, removal requires separately reviewed authority and a forward cleanup migration. No destructive production-data rollback is authorized.

**Absolute exclusions and completion boundary.** This completed unit authorizes no actual identity rows, provider, credential, signup/login, linkage mutation, immutable audit subsystem, session, recovery, API/runtime resolution, repository/service, support/admin mutation, personal-data processing, production activation, Talibeen/Editorial linkage, broader Public Identity implementation, or next unit. Its completion grants no automatic successor authority.

**Protected status truth.** Linkage Architecture remains OPEN / APPROVED; Linkage Physical Contract remains APPROVED; Durable Account Root remains COMPLETE/PASS. Product Architecture, Core Release 1, M5/M6/KE/Quran/SEO, Talibeen, and every unrelated module status remain unchanged.

**Completion evidence.** `Public ALSAMAD Authentication Identity Linkage Runtime-Inert Persistence Verified = PASS`. The unit is `COMPLETE` at commit `4056876ee738a4a1d58b03ec8081b89393f33051` (`feat: add inert authentication identity persistence`) within the exact four-file boundary above, including migration `0014_public_identity_authentication_linkage.sql` and exactly one matching journal append. Clean migration-chain verification, `npm run db:check`, typecheck, lint, repository tests (497/497), production build (265 static pages), targeted formatting, Git diff checks, zero-row proof, independent implementation review, controlled staging, commit review, and remote push verification passed. The implementation remains zero-row, runtime-inert, provider-neutral, and personal-data-processing-inactive; all provider, session, recovery, runtime, API, audit-event, retention/erasure, and real-data gates remain separately governed.
...
**Current implementation boundary.** The exact schema-free, runtime-inert Talibeen Foundation unit below is COMPLETE under `REG-0027`; no route, UI, component, schema, migration, API, authentication, account/data, discovery, messaging, notification, payment, SEO, analytics, AI, standalone client, Social Reach, charitable-campaign, next unit, or other Talibeen implementation is authorized. No physical field, enum, state machine, table, constraint, migration number, provider, deployment, or future-client implementation is assigned.

### Completed exact unit contract — TALIBEEN FOUNDATION

**Governance status.** `REG-0026` defines the maximum narrow boundary and `REG-0027` records the Owner-reviewed crossing and accepted lifecycle evidence. `TALIBEEN FOUNDATION = COMPLETE`. `Talibeen Foundation Verified = PASS`. COMPLETE means only that the exact schema-free, synthetic-only, runtime-inert three-file unit below satisfied its acceptance contract; it does not complete Talibeen overall or authorize any next unit.
...
3. **Identity:** only an opaque synthetic identity reference exists. Tests and dependency review prove no authentication, session, provider, database, production-user, or Editorial Identity dependency.
4. **Privacy:** a private Talibeen profile reference/type is not assignable or treated as a public/base ALSAMAD profile, and no public projection or serialization surface exists.
5. **Membership:** verification state varies independently from Free/Plus; Plus implies no verification, trust, ranking, compatibility, safety, religious/moral status, or preferential opportunity; no payment code exists. Premium, if named, is V2/deferred only.
6. **Runtime isolation:** no `app`, page, component, API, middleware, production composition root, or existing runtime module imports the Foundation. No barrel export is introduced. Default-off means zero route, UI, API, runtime, or real-data exposure—not a new feature-flag service.
7. **Persistence isolation:** no database, Drizzle/ORM, repository, migration, filesystem, cache, or storage import or side effect exists.
...
9. **Scope and verification:** only the three exact allowed files differ in the implementation unit. Typecheck, scoped lint, scoped format check, and the repository test command pass; the dedicated test exercises every rule above; review evidence includes a scoped diff, dependency/import search, and confirmation that quarantine is not adopted.

**Completion evidence.** `Talibeen Foundation Verified = PASS`. The unit is COMPLETE at commit `c7ba024d0d19c31c4c46ff027531d504e17bc01f` (`feat: implement Talibeen Foundation contracts`) within the exact three-file boundary. Dedicated Foundation tests are 11/11 PASS; repository tests are 497/497 PASS; TypeScript typecheck, scoped lint, scoped Prettier check, whitespace/diff check, import/dependency review, runtime isolation, persistence isolation, external isolation, and exact-scope review are PASS. Adult eligibility accepts exactly 18 and older and fails closed for under-18 or missing/invalid/malformed/non-finite/non-integer input. Candidate direction accepts man-to-woman and woman-to-man only and rejects same-sex, generic, unknown, and invalid directions. Identity remains branded, opaque, synthetic-only; private/public profile concepts remain separate; verification remains independent from Free/Plus; Plus implies no verification, trust, ranking, safety, or compatibility. Source imports and production consumers are absent; no Core table, schema, migration, API, UI, persistence, provider, or external integration changed.
...
**Release status.** Governance promotion remains `DECIDED` under `REG-0025`; the narrow Foundation exception is COMPLETE under implemented `REG-0027`. Broader Talibeen architecture contracts remain incomplete and implementation remains `BLOCKED / NOT AUTHORIZED`. No next Talibeen acceptance gate has passed and no next implementation unit is authorized.
...
## Public ALSAMAD Authentication Session Physical Contract Governance Boundary

**Governance status.** `REG-0035` and accepted `ADR-0016`, subordinate to `REG-0034`/`ADR-0015` and the durable account-root authority, approve exactly one provider-neutral, transport-neutral physical-contract governance unit. `PUBLIC ALSAMAD AUTHENTICATION SESSION PHYSICAL CONTRACT = APPROVED`. This is not an implementation crossing and creates no successor authority.

**Authoritative persistence and entity.** PostgreSQL is the authoritative persistent source of truth for ALSAMAD session state. The approved physical entity name is `user_sessions`; this approval does not create the table. A future cache or auxiliary store requires separate governance and cannot silently become canonical authority.

**Exact five-field contract.** The contract contains exactly `id`, `user_id`, `created_at`, `expires_at`, and `revoked_at`. `id` is an application-generated UUIDv7 `uuid PRIMARY KEY NOT NULL` with no database default, protected by the repository-standard UUIDv7/RFC-variant integrity approach; it is opaque, immutable, never reused, and identifies the record rather than a bearer credential. `user_id uuid NOT NULL` has no default, is immutable, and references `users(id)` through a non-deferrable update/delete-restrictive FK. `created_at timestamptz NOT NULL` and `expires_at timestamptz NOT NULL` have no defaults and are immutable; `expires_at > created_at`. `revoked_at timestamptz NULL` is initially `NULL`, must be `NULL` or at least `created_at`, and may change only once from `NULL` to a valid timestamp.

**Derived lifecycle and concurrency.** No status column exists. `ISSUED` is successful row creation; `ACTIVE` is derived only when the row is unrevoked, unexpired, and current account/authentication/security evaluation permits authority; `EXPIRED` is derived at or after `expires_at`; `REVOKED` is derived from non-null `revoked_at`; and `INVALID` remains evaluation-only. `SUSPENDED`, `updated_at`, and account/auth/session generation or version are absent. `user_id` is not unique, so multiple concurrent independently revocable rows remain possible. Exactly the primary-key index and non-unique `ix_user_sessions__user_id` index classes are approved; no other uniqueness or speculative index is approved.

**Separation and exclusions.** The contract stores no credential, raw secret, token/hash, refresh or lookup representation, rotation lineage, `user_identity_id`, device/client metadata, IP, user agent, geolocation, fingerprint, provider/contact/assurance/trust data, Recovery data, audit event, revocation reason, cleanup metadata, Talibeen identity, or Editorial identity. Credential mechanics, provider/login/signup behavior, transport/API contracts, TTL values and policy, refresh, CSRF, runtime resolution, transaction/locking/issuance serialization, revoke-all algorithms, Recovery, support/admin procedures, and cache design remain separately governed.

**Audit, privacy, and real-data gates.** Operational state remains separate from immutable audit evidence. Audit-event governance is not required for this physical decision or a future separately authorized empty runtime-inert implementation; future real issuance requires applicable audit/logging authority, real revocation requires immutable event governance where existing authority requires it, and support/admin mutation requires purpose-bound immutable evidence. Numeric retention remains unresolved. Real issuance or processing remains blocked pending applicable purpose, lawful basis, notice, retention, erasure, deletion completion, backup, access/export, correction, transfer/subprocessor, support-access, legal-hold, and jurisdiction governance.

**Absolute non-authorization.** No migration, journal, ORM/schema edit, verifier, test fixture, table creation, session row, runtime consumer, authentication middleware, token handling, API/transport behavior, provider integration, login/signup, Recovery, real data, support/admin mutation, production activation, deployment, or successor unit is authorized. Any empty, zero-row, runtime-inert persistence implementation requires a separate readiness audit, exact Roadmap implementation crossing, and explicit Owner authorization. Implementation is `BLOCKED / NOT AUTHORIZED`.

### Governance-authorized exact unit — PUBLIC ALSAMAD AUTHENTICATION SESSION RUNTIME-INERT PERSISTENCE IMPLEMENTATION

**Governance status.** `REG-0036` is `DECIDED` and governance-authorizes this exact bounded future unit as `AUTHORIZED / NOT STARTED`. Implementation is `NOT YET IMPLEMENTED / OWNER IMPLEMENTATION APPROVAL REQUIRED`. This local Registry/Roadmap write, independent governance review, controlled staging, commit authorization, governance commit, push-authorization review, explicit Owner push authorization, and publication without post-push remote verification each confer no implementation authority. Until publication to remote `main` and post-push remote verification have both completed successfully, implementation authority is `NONE` and no implementation file may be touched. Only after the complete governance chain is published and remote-verified may the Owner issue a separate explicit implementation authorization; even then, implementation does not begin automatically. `REG-0035`/accepted `ADR-0016` remain the unchanged physical authority; no `ADR-0017` is required or allocated.

**Dependency chain and objective.** `REG-0034`/`ADR-0015` → `REG-0035`/`ADR-0016` → `REG-0036`. The future unit may implement only the approved PostgreSQL `user_sessions` physical contract as an empty, zero-real-row, provider-neutral, transport-inert, API-inert, runtime-inert, and production-inert structure. Broader session/runtime and Public Identity implementation remain `BLOCKED / NOT AUTHORIZED`.
...
The migration is the mechanically next sequence after committed `0014_public_identity_authentication_linkage.sql`. The journal change is exactly one mechanical append for `0015_public_identity_authentication_session`; every prior entry and migration remains unchanged. No fifth implementation file, snapshot, standalone test, dependency/package, runtime, API, provider, Recovery, UI, service, repository, or unrelated file is authorized. Any additional file or material physical decision requires implementation to stop and return to governance.

**Exact physical unit.** PostgreSQL remains authoritative. Create exactly `user_sessions` with exactly five columns and no sixth field: immutable, opaque, never-reused, application-generated UUIDv7 record identity `id uuid PRIMARY KEY NOT NULL` with no database default and repository-standard RFC-variant integrity; immutable non-unique `user_id uuid NOT NULL` with no default and a `NOT DEFERRABLE` FK to `users(id)` using `ON UPDATE RESTRICT` and `ON DELETE RESTRICT`; immutable `created_at timestamptz NOT NULL` with no default; immutable `expires_at timestamptz NOT NULL` with no default and `expires_at > created_at`; and nullable `revoked_at timestamptz`, initially `NULL`, with no default, constrained by `revoked_at IS NULL OR revoked_at >= created_at`, and mutable only once from `NULL` to a valid timestamp. `id` is record identity and never bearer identity.

**Database integrity and lifecycle.** Database constraints and `trg_user_sessions__integrity`, not application convention, enforce the contract. The trigger rejects changes to `id`, `user_id`, `created_at`, and `expires_at`; rejects clearing or rewriting non-null `revoked_at`; permits only `revoked_at: NULL → valid timestamp`; and rejects no-op lifecycle mutation. No status/lifecycle column exists. `ISSUED` is successful creation; `ACTIVE` requires unrevoked, unexpired state plus current fail-closed account/authentication/security evaluation; `EXPIRED` is current time at or after expiry; `REVOKED` is non-null `revoked_at`; `INVALID` remains evaluation-only; and `SUSPENDED` remains absent.

**Indexes and concurrency.** The exact indexes are `user_sessions_pkey` and non-unique `ix_user_sessions__user_id`. Primary-key uniqueness is the only uniqueness rule. Multiple rows for one user remain permitted. No expiry, revocation, token, provider, cleanup, or speculative operational index is authorized.

**Zero-row and inert acceptance.** No seed, backfill, retained fixture, imported, synthetic-retained, real, or production row may exist. Transaction-scoped verifier rows must roll back, and post-verification persistent row count must equal zero. Runtime scanning must prove zero consumers in application routes, components, server actions, API handlers, services, repositories, authentication/provider flows, Recovery, background jobs, and composition roots. The unit creates no issuance, validation, revocation, revoke-all, account-wide mutation, login/signup, credential resolution, token handling, cookie/header/endpoint/middleware/serialization/CSRF mechanism, API surface, provider integration, deployment, or production effect.

**Credential and prohibited-data boundary.** No raw bearer/session secret, session token, token hash, refresh value, lookup secret, credential-resolution persistence, rotation lineage, or replay mechanism is stored or selected. The five-column catalog contains no status, `updated_at`, generation/version, `user_identity_id`, credential, IP, user agent, location, fingerprint, device identifier/label, provider metadata/token, contact identifier, assurance/trust/reputation/personhood/household data, Talibeen or Editorial identity, audit-event reference, revocation reason, cleanup metadata, or other field.

**Acceptance contract.** Real PostgreSQL and repository verification must prove exact table/column/type/nullability/default shape; UUIDv7/RFC acceptance and invalid version/variant rejection; missing and duplicate ID rejection; immutable ID; owner presence and existence; restrictive non-deferrable FK; immutable non-unique `user_id`; multiple sessions for one user; required immutable timestamps; equal/earlier expiry rejection and later expiry acceptance; nullable and temporally valid `revoked_at`; one-way revocation and clearing/rewriting/no-op rejection; exactly two indexes and no extra uniqueness; prohibited-field/raw-secret absence; zero rows; exact migration and one-entry journal append; prior-migration integrity; exact four-file boundary; zero runtime/API/provider/Recovery consumers; and rollback readiness. All synthetic verifier rows are transaction-scoped and rolled back.

Required quality evidence is `npm run db:check`, `npm run typecheck`, `npm run lint`, targeted implementation-file formatting, repository tests, `npm run build`, and `git diff --check`, plus exact implementation/staged-file review and rollback evidence. Unrelated repository-wide formatting debt must be disclosed and must not widen this unit.

**Rollback.** Migration failure rolls back atomically. Verification failure leaves zero retained rows and prevents acceptance. Before application, an unaccepted local implementation may be reverted within the exact boundary. Once committed or applied, migration and journal history are never rewritten; removal while empty and unconsumed requires separate authority and a forward cleanup migration.

**Audit, privacy, and downstream gates.** Immutable audit-event governance and retention/privacy policy are not prerequisites to this empty inert unit. Applicable audit/logging remains mandatory before real issuance and applicable lifecycle/support/admin operations. Real processing remains blocked pending applicable purpose, lawful basis, notice, retention, erasure, deletion completion, backup, access/export, correction, transfers/subprocessors, support access, legal hold, and jurisdiction governance. Recovery remains separate/downstream and unauthorized. Provider/login/signup and API/transport remain deferred and unauthorized. Existing no-surface authority remains controlling.

**Absolute non-authority and completion boundary.** Actual implementation remains unauthorized until separate explicit Owner approval. REG-0036 authorizes no real data; issuance; validation, revocation, revoke-all, or account-wide runtime mutation; bearer credential, JWT, opaque-token, hashing/storage, refresh, rotation, or replay implementation; provider/login/signup; API/transport, cookies, headers, endpoints, middleware, or serialization; Recovery; immutable audit-event implementation; support/admin mutation; production activation/deployment; broader Public Identity implementation; staging, commit, or push; or successor governance. Completion requires separately reviewed implementation evidence and a later Registry/Roadmap status update and grants no automatic successor authority.
