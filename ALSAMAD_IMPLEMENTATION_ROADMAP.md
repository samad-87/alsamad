...
**Status and dependency truth.** KE-1 remains COMPLETE. KE-2 governance remains COMPLETE; REG-0024/ADR-0010 supersede REG-0023's incomplete crossing and authorize KE-2A implementation to begin in a separate future execution against the corrected contract, but KE-2A implementation remains NOT STARTED; KE-2B implementation remains NOT STARTED / BLOCKED and still requires its own later Governance Unit 2 crossing. This prerequisite is now implemented at commit `a2604ca` and removes the `editorial_users` blocker for both units; `devotional_items` remains a hard independent blocker only for KE-2B and does not block KE-2A. M5 Gate 3 remains PARTIAL; M5 Provider Import Dry Run Verified and M5 Quran Import Activated remain NOT PASS; M6 remains BLOCKED; Duas governance is otherwise unchanged by this prerequisite (`REG-0019`'s separate, narrow M6.0-only M5-dependency exception is unrelated to this entry).
...
**Status.** `M7.0-track` gate structure is recorded. `REG-0014` authorized exactly KE-1, which is now COMPLETE at `e073879`; it remains runtime-inert. `REG-0022`/`ADR-0009` remain the current governing architecture and sequencing authority for KE-2's unchanged later-additive two-table contract. `REG-0024`/`ADR-0010` supersede historical REG-0023's incomplete crossing and supply only KE-2A's corrected implementation authorization: KE-2A is authorized to begin in a separate future execution but implementation remains NOT STARTED; KE-2B implementation remains NOT STARTED / BLOCKED and unauthorized pending its own later crossing. `ADR-0007` is historical/Superseded; its §§1–3 remain binding only through `ADR-0009`'s incorporation by reference. Phase 3 onward, Duas, collections/references, generic persisted knowledge edges, runtime UI/search wiring, and AI remain NOT AUTHORIZED. No prototype byte carries authority merely by existing, including the current untracked `src/lib/knowledge/topics.ts`. `M5 Gate 3` remains `PARTIAL`, M5 Gates 4–7 remain `NOT PASS`, `M5 Provider Import Dry Run Verified` and `M5 Quran Import Activated` remain `NOT PASS`, `M6.0` remains `COMPLETE`, and `M6.1`/`M6.2` remain `BLOCKED`. `REG-0019`, `REG-0020`, Sakīnah, Editorial Identity Foundation, and Duas semantics and statuses are unchanged.

#### M7.0-track / KE-2A — Topics Foundation

**Governance status.** `REG-0022`/`ADR-0009` govern the delivery split and incorporate the unchanged `topics` physical specification from historical/Superseded `ADR-0007` §2; they do **not** authorize implementation. `REG-0024`/`ADR-0010` supersede historical/Superseded `REG-0023` and satisfy KE-2A's corrected Governance Unit 2 crossing. They authorize implementation to begin only in a separate future execution against this corrected contract. `KE-2A` implementation remains **NOT STARTED**; no migration execution, schema/code implementation, journal append, staging, commit, push, seed data, or runtime wiring occurs in this governance task.
...
**Exact future implementation boundary.** A future, separately authorized KE-2A execution may touch only:

```
src/db/schema.ts
drizzle/<next-authorized-forward-number>_ke2a_topics.sql
drizzle/meta/_journal.json
...
The KE-2A migration number is assigned mechanically only at implementation time from the then-authoritative forward-only sequence; it is not assigned, frozen, or pre-claimed here and may not reuse or displace `0010_devotional_content_foundation.sql`. At the current baseline, `0011` exists and `0012` is only the observed mechanically lawful result if execution begins from this unchanged baseline; implementation must recompute the number from its then-authoritative state. `drizzle/meta/_journal.json` may receive exactly one mechanical append registering that migration. Every existing journal entry remains byte-unchanged; the append contains only the next journal index, established version/breakpoint fields, a mechanically monotonic timestamp, and the tag derived exactly from the assigned migration filename. The existing uncommitted `src/lib/knowledge/topics.ts` is prototype evidence only and requires a full conformance rewrite against the incorporated specification before any authorized inclusion. No other prototype file is authorized. KE-2A must not implement `content_topics`.
...
**Completion evidence.** Only a later implementation commit satisfying every acceptance item assigned to a unit may mark that unit COMPLETE. Governance approval alone must continue to report `KE-2A implementation = NOT STARTED` and `KE-2B implementation = NOT STARTED / BLOCKED`.
