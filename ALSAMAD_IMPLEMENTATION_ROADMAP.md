...
**Frozen sources, derivation, and delivery.** The official releases provide authoritative variable TTF sources, not native WOFF2 assets: `NotoSansArabic/full/variable-ttf/NotoSansArabic[wdth,wght].ttf` from `NotoSansArabic-v2.013.zip` and `NotoNaskhArabic/full/variable-ttf/NotoNaskhArabic[wght].ttf` from `NotoNaskhArabic-v2.021.zip`. Their archive SHA-256 values remain those frozen in REG-0017. The later implementation may produce and commit only the deterministic unsubsetted derivatives `NotoSansArabic[wdth,wght]-v2.013.woff2` at weights 400–800/default width and the regular 400 use of `NotoNaskhArabic[wght]-v2.021.woff2`.
...
**Reproducibility and integrity evidence.** Before adoption, implementation must: verify each official archive SHA-256; extract the exact governed TTF; record its SHA-256; record Python patch, FontTools, and Brotli versions plus the exact command; convert twice in separate clean temporary directories; require bit-identical derived SHA-256 outputs; and record the final WOFF2 SHA-256. Any mismatch fails closed. Source-to-derived/decompressed comparison must verify `fvar`, `avar` where present, `gvar` where present, `GDEF`, `GSUB`, `GPOS`, `cmap`, `name`, `OS/2`, relevant hint/layout tables, Arabic shaping, mark positioning, and representative Arabic/tashkeel output. Expected WOFF2 table ordering, `head` flag, and applicable `DSIG` differences are permitted. Each derived font remains under SIL OFL 1.1, must ship with the applicable copyright and complete license notice, preserve family metadata, and must not imply upstream endorsement. Both fonts are self-hosted locally with no runtime third-party request. Safe fallbacks remain `Tahoma, Arial, sans-serif` and `serif` respectively.
...
**Acceptance.** The later implementation must pass the full reproducibility and integrity evidence contract above and prove that both derived fonts resolve locally at build/runtime with no runtime third-party request; Arabic UI receives Noto Sans Arabic; committed devotional-reading surfaces receive Noto Naskh Arabic; English UI inherits neither the devotional nor Quran role; `--font-quran` remains unbound; the exact sources, derived assets, licenses, versions, commands, and checksums match REG-0017; Arabic/Quran text code points remain unchanged; representative Arabic and tashkeel do not clip; and no Quran font activates. Real-browser visual verification is required for shaping and clipping because static or DOM-emulated tests cannot establish those properties.
