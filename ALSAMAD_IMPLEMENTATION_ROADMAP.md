...
**Governance treatment and status truth.** No Registry entry or ADR is required: this is a reversible provider-specific implementation behind the accepted injected transport boundary and changes no persisted representation, canonical ownership, licensing decision, or difficult-to-reverse architecture. M5.2D governance and offline implementation are COMPLETE at commit `8810b36`; real provider token validation remains NOT EXECUTED until the separate M5.2E execution authorization below is committed and then explicitly exercised. `M5 Schema Foundation Verified` remains PASS; M5 Gate 3 remains PARTIAL/incomplete; Gate 4 remains NOT STARTED/incomplete; `M5 Provider Import Dry Run Verified` and `M5 Quran Import Activated` remain NOT PASS; M6 remains BLOCKED; KE-2 and Phase 7 implementation remain NOT STARTED. M5.2D completion changes none of those statuses.

### M5.2E — One Controlled Pre-Production Token Validation Attempt

**Execution authorization.** After this Roadmap authorization is committed, exactly one later execution may run `npm run quran:token-check` once with `QURAN_FOUNDATION_ENVIRONMENT=sandbox`, the locally controlled Pre-Production credentials whose M5.2B/M5.2C structural availability evidence is PASS, and the committed M5.2D implementation at `8810b36`. That single CLI invocation may issue exactly one outbound provider request: `POST https://prelive-oauth2.quran.foundation/oauth2/token`. It uses the committed HTTP Basic, form body, exact-origin, redaction, response-validation, finite-timeout, one-request, zero-retry, memory-only token, and adapter-close contracts. No second invocation or second real token request is authorized by this unit.
...
**Order, acceptance, and status truth.** The binding order is: controlled credential availability PASS; this committed one-attempt authorization; exactly one bounded token validation; evidence evaluation; remaining legal/license Gate 3 work; separate Gate 4 source selection; separately authorized controlled Content API access/fetch; and only then later dry-run and activation gates. Acceptance requires evidence proving one token endpoint attempt at most, no retry, the fixed Pre-Production host, Production exclusion, complete redaction, zero Content API/metadata/resource/database activity, immediate token disposal, and unchanged downstream statuses. Governance authoring alone does not execute the request. After a future PASS, M5 Gate 3 remains PARTIAL/incomplete; Gate 4 remains NOT STARTED/incomplete; `M5 Provider Import Dry Run Verified` and `M5 Quran Import Activated` remain NOT PASS; M6 remains BLOCKED; and KE-2 and Phase 7 implementation remain NOT STARTED.
