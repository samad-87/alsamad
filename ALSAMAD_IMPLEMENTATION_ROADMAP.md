...
Migration `0002` is immutable and must not be edited. Migration `0005` is the next forward migration because committed migrations end at `0004`; it adds no table and preserves the 30-table Release 1 freeze. M6's future devotional migration, originally moved to `0006_devotional_content_foundation.sql` by this correction, then to `0007_devotional_content_foundation.sql` by the ARC-004 correction at `0006`, then to `0008_devotional_content_foundation.sql` following the AUD-001 correction at `0007` (see AUD-001 below), now moves to `0009_devotional_content_foundation.sql` following the ARC-005 reservation at `0008` (see ARC-005 below). This numbering correction does not authorize M6.
...
Migration `0004` is immutable and must not be edited. Migration `0007` is the next forward migration because committed migrations end at `0006`; it adds no table and preserves the 30-table Release 1 freeze. M6's future devotional migration moved from `0007_devotional_content_foundation.sql` to `0008_devotional_content_foundation.sql`, the next available number after this correction, and now moves to `0009_devotional_content_foundation.sql` following the ARC-005 reservation at `0008` (see ARC-005 below). This numbering correction does not authorize ARC-005 or M6.
...
Migration `0007` is immutable and must not be edited. Migration `0008` is reserved for this decision's future implementation, the next available forward migration after AUD-001 at `0007`; it adds no table and preserves the 30-table Release 1 freeze. M6's future devotional migration moves from `0008_devotional_content_foundation.sql` to `0009_devotional_content_foundation.sql`, the next available number after this reservation (see Phase 6 below). This numbering reservation does not authorize ARC-005 implementation, ARC-006, or M6.
...
**Status and handoff:** ARC-005 is decided and specified but not implemented. `M5 Schema Foundation Verified` remains PASS; ARC-001, ARC-002, ARC-003, ARC-004, and AUD-001 remain complete or implemented as previously recorded. `M5 Provider Import Dry Run Verified` and `M5 Quran Import Activated` remain NOT PASS, and M6 remains blocked, independent of this decision. This decision does not select a source, approve credentials, authorize provider access, create a real manifest, permit a fetch or dry run, or approve publication of any real content.

**Dependency boundaries:** ARC-005 is architecturally independent of ARC-004 and of ARC-006 (stale M6 prerequisite). It depends on AUD-001's completion as a precondition (satisfied, above) but does not itself depend on ARC-002/ARC-003 conformance work. Neither ARC-006 nor M6 is solved or authorized here.
...
- `src/db/schema.ts`
- `drizzle/0009_devotional_content_foundation.sql`
- `drizzle/meta/_journal.json`
- `scripts/db-verify.mjs`
...
**Objective.** Implement exactly the four Release 1 devotional tables — `devotional_items`, `devotional_collections`, `devotional_collection_items`, `content_translations` — in dependency order, register forward migration `0009`, and prove the Schema completion gate on real PostgreSQL.
